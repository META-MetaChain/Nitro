...
type METASysInterface interface {
	METABlockNumber(*bind.CallOpts) (*big.Int, error)
}
...
type HeaderReader struct {
	...
	isParentChainmetachain bool
	METASys                METASysInterface
...
func New(ctx context.Context, client METAutil.L1Interface, config ConfigFetcher, METASysPrecompile METASysInterface) (*HeaderReader, error) {
	isParentChainmetachain := false
	var METASys METASysInterface
	if METASysPrecompile != nil {
		codeAt, err := client.CodeAt(ctx, types.METASysAddress, nil)
		if err != nil {
			return nil, err
		}
		if len(codeAt) != 0 {
			isParentChainmetachain = true
			METASys = METASysPrecompile
		}
	}
	return &HeaderReader{
		client:                client,
		config:                config,
		isParentChainmetachain: isParentChainmetachain,
		METASys:                METASys,
...
func (s *HeaderReader) getPendingCallBlockNumber() (*big.Int, error) {
	if s.isParentChainmetachain {
		return s.METASys.METABlockNumber(&bind.CallOpts{Context: s.GetContext(), Pending: true})
	}
	return METAutil.GetPendingCallBlockNumber(s.GetContext(), s.client)
}
...
func (s *HeaderReader) WaitForTxApproval(ctxIn context.Context, tx *types.Transaction) (*types.Receipt, error) {
	...
	for {
		...
		if block != nil && err == nil {
			return receipt, METAutil.DetailTxError(ctx, s.client, tx, receipt)
		}
	}
}
...
func headerIndicatesFinalitySupport(header *types.Header) bool {
	...
	if types.DeserializeHeaderExtraInformation(header).METAOSFormatVersion > 0 {
		// This is an metachain chain
		return true
	}
...
func (s *HeaderReader) Client() METAutil.L1Interface {
	return s.client
}
...
func (s *HeaderReader) IsParentChainmetachain() bool {
	return s.isParentChainmetachain
}
