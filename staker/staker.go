...
import (
	...
	"github.com/META-MetaChain/nitro/staker/txbuilder"
...
var (
	stakerBalanceGauge              = metrics.NewRegisteredGaugeFloat64("META/staker/balance", nil)
	stakerAmountStakedGauge         = metrics.NewRegisteredGauge("META/staker/amount_staked", nil)
	stakerLatestStakedNodeGauge     = metrics.NewRegisteredGauge("META/staker/staked_node", nil)
	stakerLatestConfirmedNodeGauge  = metrics.NewRegisteredGauge("META/staker/confirmed_node", nil)
	stakerLastSuccessfulActionGauge = metrics.NewRegisteredGauge("META/staker/action/last_success", nil)
	stakerActionSuccessCounter      = metrics.NewRegisteredCounter("META/staker/action/success", nil)
	stakerActionFailureCounter      = metrics.NewRegisteredCounter("META/staker/action/failure", nil)
	validatorGasRefunderBalance     = metrics.NewRegisteredGaugeFloat64("META/validator/gasrefunder/balanceether", nil)
)
...
type LatestStakedNotifier interface {
	UpdateLatestStaked(count METAutil.MessageIndex, globalState validator.GoGlobalState)
}

type LatestConfirmedNotifier interface {
	UpdateLatestConfirmed(count METAutil.MessageIndex, globalState validator.GoGlobalState)
}
...
func (s *Staker) getLatestStakedState(ctx context.Context, staker common.Address) (uint64, METAutil.MessageIndex, *validator.GoGlobalState, error) {
	...
	s.CallIteratively(func(ctx context.Context) (returningWait time.Duration) {
		...
		if common.HexToAddress(s.config.GasRefunderAddress) != (common.Address{}) {
			...
			if err != nil {
				...
				validatorGasRefunderBalance.Update(METAmath.BalancePerEther(gasRefunderBalance))
			}
		}
		...
		METATx, err := s.Act(ctx)
		if err == nil && METATx != nil {
			_, err = s.l1Reader.WaitForTxApproval(ctx, METATx)
			if err == nil {
				log.Info("successfully executed staker transaction", "hash", METATx.Hash())
			} else {
		...
		if err == nil {
			...
			if METATx != nil && !s.wallet.CanBatchTxs() {
...
func (s *Staker) Act(ctx context.Context) (*types.Transaction, error) {
	...
	if shouldResolveNodes {
		METATx, err := s.resolveTimedOutChallenges(ctx)
		if err != nil {
...
func (s *Staker) updateStakerBalanceMetric(ctx context.Context) {
	...
	stakerBalanceGauge.Update(METAmath.BalancePerEther(balance))
}
