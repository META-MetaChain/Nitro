...
var (
	validatorPendingValidationsGauge  = metrics.NewRegisteredGauge("META/validator/validations/pending", nil)
	validatorValidValidationsCounter  = metrics.NewRegisteredCounter("META/validator/validations/valid", nil)
	validatorFailedValidationsCounter = metrics.NewRegisteredCounter("META/validator/validations/failed", nil)
	validatorMsgCountCurrentBatch     = metrics.NewRegisteredGauge("META/validator/msg_count_current_batch", nil)
	validatorMsgCountValidatedGauge   = metrics.NewRegisteredGauge("META/validator/msg_count_validated", nil)
)
...
type BlockValidator struct {
	...
	*StatelessBlockValidator
	...
	nextCreateBatchMsgCount METAutil.MessageIndex
	...
	valLoopPos      METAutil.MessageIndex
	...
	recordSentA uint64
	validatedA  uint64
	validations containers.SyncMap[METAutil.MessageIndex, *validationStatus]
...
func atomicStorePos(addr *uint64, val METAutil.MessageIndex) {
	atomic.StoreUint64(addr, uint64(val))
}

func atomicLoadPos(addr *uint64) METAutil.MessageIndex {
	return METAutil.MessageIndex(atomic.LoadUint64(addr))
}

func (v *BlockValidator) created() METAutil.MessageIndex {
	return atomicLoadPos(&v.createdA)
}

func (v *BlockValidator) recordSent() METAutil.MessageIndex {
	return atomicLoadPos(&v.recordSentA)
}

func (v *BlockValidator) validated() METAutil.MessageIndex {
	return atomicLoadPos(&v.validatedA)
}

func (v *BlockValidator) Validated(t *testing.T) METAutil.MessageIndex {
	return v.validated()
}
...
func GlobalStateToMsgCount(tracker InboxTrackerInterface, streamer TransactionStreamerInterface, gs validator.GoGlobalState) (bool, METAutil.MessageIndex, error) {
	...
	var prevBatchMsgCount METAutil.MessageIndex
	if gs.Batch > 0 {
	...
	if gs.PosInBatch > 0 {
		...
		count += METAutil.MessageIndex(gs.PosInBatch)
...
func (v *BlockValidator) readBatch(ctx context.Context, batchNum uint64) (bool, []byte, METAutil.MessageIndex, error) {
...
func (v *BlockValidator) createNextValidationEntry(ctx context.Context) (bool, error) {
	...
	pos := v.created()
	if pos > v.validated()+METAutil.MessageIndex(v.config().ForwardBlocks) {
...
func (v *BlockValidator) sendNextRecordRequests(ctx context.Context) (bool, error) {
	...
	recordUntil := validated + METAutil.MessageIndex(v.config().PrerecordedBlocks) - 1
...
func (v *BlockValidator) iterativeValidationPrint(ctx context.Context) time.Duration {
	...
	var batchMsgs METAutil.MessageIndex
...
func (v *BlockValidator) advanceValidations(ctx context.Context) (*METAutil.MessageIndex, error) {
	v.reorgMutex.RLock()
	defer v.reorgMutex.RUnlock()
...
func (v *BlockValidator) UpdateLatestStaked(count METAutil.MessageIndex, globalState validator.GoGlobalState) {
...
func (v *BlockValidator) Reorg(ctx context.Context, count METAutil.MessageIndex) error {
	v.reorgMutex.Lock()
	defer v.reorgMutex.Unlock()
	if count <= 1 {
...
func (v *BlockValidator) checkLegacyValid() error {
	...
	var msgCount METAutil.MessageIndex
	...
	msgCount += METAutil.MessageIndex(v.legacyValidInfo.AfterPosition.PosInBatch)
...
func (v *BlockValidator) WaitForPos(t *testing.T, ctx context.Context, pos METAutil.MessageIndex, timeout time.Duration) bool {
