...
func (v *L1Validator) generateNodeAction(
	...
	startStateProposedHeader, err := v.client.HeaderByNumber(ctx, METAmath.UintToBig(startStateProposedParentChain))
	if err != nil {
		return nil, false, fmt.Errorf(
	...
	var validatedCount METAutil.MessageIndex
	...
	l1BlockNumber, err := METAutil.CorrespondingL1BlockNumber(ctx, v.client, currentL1BlockNum)
	if err != nil {
		return nil, false, err
	}
...
func (v *L1Validator) createNewNodeAction(
	...
	stakerInfo *OurStakerInfo,
	prevInboxMaxCount *big.Int,
	startCount METAutil.MessageIndex,
	startState *validator.ExecutionState,
	validatedCount METAutil.MessageIndex,
...
func lookupNodeStartState(ctx context.Context, rollup *RollupWatcher, nodeNum uint64, nodeHash common.Hash) (*validator.ExecutionState, *big.Int, uint64, uint64, error) {
	if nodeNum == 0 {
		...
		l1BlockNumber, err := METAutil.CorrespondingL1BlockNumber(ctx, rollup.client, creationEvent.Raw.BlockNumber)
		if err != nil {
