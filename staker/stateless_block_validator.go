...
type StatelessBlockValidator struct {
	...
	db           ethdb.Database
	daService    METAstate.DataAvailabilityReader
...
type InboxTrackerInterface interface {
	...
	GetBatchMessageCount(seqNum uint64) (METAutil.MessageIndex, error)
...
type TransactionStreamerInterface interface {
	BlockValidatorRegistrer
	GetProcessedMessageCount() (METAutil.MessageIndex, error)
	GetMessage(seqNum METAutil.MessageIndex) (*METAostypes.MessageWithMetadata, error)
	ResultAtCount(count METAutil.MessageIndex) (*execution.MessageResult, error)
	PauseReorgs()
	ResumeReorgs()
}
...
func GlobalStatePositionsAtCount(
	...
	var firstInBatch METAutil.MessageIndex
	if batch > 0 {
...
func FindBatchContainingMessageIndex(
	tracker InboxTrackerInterface, pos METAutil.MessageIndex, high uint64,
) (uint64, error) {
	var low uint64
...
func (v *StatelessBlockValidator) ValidationEntryRecord(ctx context.Context, e *validationEntry) error {
	...
	e.Preimages = make(map[METAutil.PreimageType]map[common.Hash][]byte)
	if e.Pos != 0 {
		...
		if recording.Preimages != nil {
			e.Preimages[METAutil.Keccak256PreimageType] = recording.Preimages
		}
	}
	if e.HasDelayedMsg {
	...
	for _, batch := range e.BatchInfo {
		...
		if !METAstate.IsDASMessageHeaderByte(batch.Data[40]) {
			continue
		}
		if v.daService == nil {
			log.Warn("No DAS configured, but sequencer message found with DAS header")
		} else {
			_, err := METAstate.RecoverPayloadFromDasBatch(
				ctx, batch.Number, batch.Data, v.daService, e.Preimages, METAstate.KeysetValidate,
			)
			if err != nil {
				return err
			}
...
// return the globalState position before and after processing message at the specified count
func (v *StatelessBlockValidator) GlobalStatePositionsAtCount(count METAutil.MessageIndex) (GlobalStatePosition, GlobalStatePosition, error) {
	if count == 0 {
...
func (v *StatelessBlockValidator) CreateReadyValidationEntry(ctx context.Context, pos METAutil.MessageIndex) (*validationEntry, error) {
...
func (v *StatelessBlockValidator) ValidateResult(
	ctx context.Context, pos METAutil.MessageIndex, useExec bool, moduleRoot common.Hash,
