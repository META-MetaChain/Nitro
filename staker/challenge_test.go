...
func CreateChallenge(
	...
	inputMachine server_META.MachineInterface,
...
func runChallengeTest(
	t *testing.T,
	baseMachine *server_META.METAitratorMachine,
	incorrectMachine server_META.MachineInterface,
	...
	var asserterMachine, challengerMachine server_META.MachineInterface
	...
	asserterRun, err := server_META.NewExecutionRun(ctx,
		func(context.Context) (server_META.MachineInterface, error) { return asserterMachine, nil },
		&server_META.DefaultMachineCacheConfig)
	Require(t, err)
	...
	challengerRun, err := server_META.NewExecutionRun(ctx,
		func(context.Context) (server_META.MachineInterface, error) { return challengerMachine, nil },
		&server_META.DefaultMachineCacheConfig)
	Require(t, err)
...
func createBaseMachine(t *testing.T, wasmname string, wasmModules []string) *server_META.METAitratorMachine {
	_, filename, _, _ := runtime.Caller(0)
	wasmDir := path.Join(path.Dir(filename), "../METAitrator/prover/test-cases/")
	...
	machine, err := server_META.LoadSimpleMachine(wasmPath, modulePaths)
	Require(t, err)
...
func TestChallengeToOSP(t *testing.T) {
	...
	IncorrectMachine := server_META.NewIncorrectMachine(machine, 200)
...
func TestChallengeToErroredOSP(t *testing.T) {
	...
	IncorrectMachine := server_META.NewIncorrectMachine(machine, 10)
	runChallengeTest(t, machine, IncorrectMachine, false, false, 0)
}
