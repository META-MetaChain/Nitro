...
func (v *Contract) ExecuteTransactions(ctx context.Context, builder *txbuilder.Builder, gasRefunder common.Address) (*types.Transaction, error) {
	...
	if len(txes) == 1 {
		METATx, err := v.executeTransaction(ctx, txes[0], gasRefunder)
		if err != nil {
			return nil, err
		}
	...
	METATx, err := v.dataPoster.PostTransaction(ctx, time.Now(), auth.Nonce.Uint64(), nil, *v.Address(), txData, gas, auth.Value, nil)
	if err != nil {
		return nil, err
	}
	builder.ClearTransactions()
...
func (v *Contract) estimateGas(ctx context.Context, value *big.Int, data []byte) (uint64, error) {
	...
	gasFeeCap = METAmath.BigMax(gasFeeCap, METAmath.FloatToBig(params.GWei))
...
func (v *Contract) L1Client() METAutil.L1Interface {
