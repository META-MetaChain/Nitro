...
func NewBlockChallengeBackend(
	...
	var startMsgCount METAutil.MessageIndex
	if startGs.Batch > 0 {
		var err error
	...
	startMsgCount += METAutil.MessageIndex(startGs.PosInBatch)

	var endMsgCount METAutil.MessageIndex
	if maxBatchesRead > 0 {
		var err error
...
func (b *BlockChallengeBackend) findBatchAfterMessageCount(msgCount METAutil.MessageIndex) (uint64, error) {
	if msgCount == 0 {
		return 0, nil
	}
	low := b.startGs.Batch
...
func (b *BlockChallengeBackend) FindGlobalStateFromMessageCount(count METAutil.MessageIndex) (validator.GoGlobalState, error) {
...
func (b *BlockChallengeBackend) GetMessageCountAtStep(step uint64) METAutil.MessageIndex {
	return b.startMsgCount + METAutil.MessageIndex(step)
}
