...
func testReceiveMessages(t *testing.T, clientCompression bool, serverCompression bool, serverRequire bool, expectNoMessagesReceived bool) {
	...
	go func() {
		for i := 0; i < messageCount; i++ {
			Require(t, b.BroadcastSingle(METAostypes.TestMessageWithMetadataAndRequestId, METAutil.MessageIndex(i)))
		}
	}()
...
func newTestBroadcastClient(config Config, listenerAddress net.Addr, chainId uint64, currentMessageCount METAutil.MessageIndex, txStreamer TransactionStreamerInterface, confirmedSequenceNumberListener chan METAutil.MessageIndex, feedErrChan chan error, validAddr *common.Address) (*BroadcastClient, error) {
...
func TestServerClientDisconnect(t *testing.T) {
	...
	Require(t, b.BroadcastSingle(METAostypes.EmptyTestMessageWithMetadata, 0))
...
func TestBroadcastClientConfirmedMessage(t *testing.T) {
	...
	confirmedSequenceNumberListener := make(chan METAutil.MessageIndex, 10)
	ts := NewDummyTransactionStreamer(chainId, nil)
	broadcastClient, err := newTestBroadcastClient(
		DefaultTestConfig,
		b.ListenerAddr(),
	...
	confirmNumber := METAutil.MessageIndex(42)
	b.Confirm(42)
...
func TestBroadcasterSendsCachedMessagesOnClientConnect(t *testing.T) {
	...
	*/
	...
	Require(t, b.BroadcastSingle(METAostypes.EmptyTestMessageWithMetadata, 0))
	Require(t, b.BroadcastSingle(METAostypes.EmptyTestMessageWithMetadata, 1))
