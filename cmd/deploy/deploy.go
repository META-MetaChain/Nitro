...
import (
	...
	deploycode "github.com/META-MetaChain/nitro/deploy"
)
...
func main() {
	...
	l1Reader, err := headerreader.New(ctx, l1client, func() *headerreader.Config { return &headerReaderConfig }, METASys)
	if err != nil {
	...
	deployedAddresses, err := deploycode.DeployOnL1(
		...
		sequencerAddress,
		*authorizevalidators,
		METAnode.GenerateRollupConfig(*prod, moduleRoot, ownerAddress, &chainConfig, chainConfigJson, loserEscrowAddress),
		nativeToken,
		maxDataSize,
	)
	if err != nil {
	...
	parentChainIsmetachain := l1Reader.IsParentChainmetachain()
	chainsInfo := []chaininfo.ChainInfo{
		{
			ChainName:             *l2ChainName,
			ParentChainId:         l1ChainId.Uint64(),
			ParentChainIsmetachain: &parentChainIsmetachain,
