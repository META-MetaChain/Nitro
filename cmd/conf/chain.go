...
var L2ConfigDefault = L2Config{
	ID:                   0,
	Name:                 "",
	InfoFiles:            []string{}, // Default file used is chaininfo/metachain_chain_info.json, stored in DefaultChainInfo in chain_info.go
	InfoJson:             "",
...
func L2ConfigAddOptions(prefix string, f *flag.FlagSet) {
	f.Uint64(prefix+".id", L2ConfigDefault.ID, "L2 chain ID (determines metachain network)")
	f.String(prefix+".name", L2ConfigDefault.Name, "L2 chain name (determines metachain network)")
