...
import (
	...
	"github.com/META-MetaChain/nitro/util/metricsutil"
...
const metricBaseOldHash = "META/das/dataavailability/oldhash/"
const metricBaseNewHash = "META/das/dataavailability/oldhash/"
...
type DataAvailabilityCheck struct {
	...
	urlToReaderMap map[string]METAstate.DataAvailabilityReader
...
func newDataAvailabilityCheck(ctx context.Context, dataAvailabilityCheckConfig *DataAvailabilityCheckConfig) (*DataAvailabilityCheck, error) {
	...
	urlToReaderMap := make(map[string]METAstate.DataAvailabilityReader, len(onlineUrls))
...
func (d *DataAvailabilityCheck) checkDataAvailability(ctx context.Context, deliveredLog types.Log, metricBase string) (bool, error) {
	...
	cert, err := METAstate.DeserializeDASCertFrom(bytes.NewReader(data))
	if err != nil {
		return true, err
	}
