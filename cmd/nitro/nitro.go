...
import (
	...
	"github.com/META-MetaChain/nitro/METAnode/resourcemanager"
	"github.com/META-MetaChain/nitro/METAutil"
	"github.com/META-MetaChain/nitro/cmd/chaininfo"
	"github.com/META-MetaChain/nitro/cmd/conf"
	"github.com/META-MetaChain/nitro/cmd/genericconf"
	"github.com/META-MetaChain/nitro/cmd/util"
	"github.com/META-MetaChain/nitro/cmd/util/confighelpers"
	...
	"github.com/META-MetaChain/nitro/solgen/go/rollupgen"
	"github.com/META-MetaChain/nitro/staker"
	"github.com/META-MetaChain/nitro/staker/validatorwallet"
	"github.com/META-MetaChain/nitro/util/colors"
	"github.com/META-MetaChain/nitro/util/headerreader"
	"github.com/META-MetaChain/nitro/util/rpcclient"
...
func mainImpl() int {
	...
	log.Info("Running metachain nitro node", "revision", vcsRevision, "vcs.time", vcsTime)
	...
	defaultBatchPosterL1WalletConfig := METAnode.DefaultBatchPosterL1WalletConfig
	...
	if nodeConfig.Execution.RPC.MaxRecreateStateDepth == metachain.UninitializedMaxRecreateStateDepth {
		if nodeConfig.Execution.Caching.Archive {
			nodeConfig.Execution.RPC.MaxRecreateStateDepth = metachain.DefaultArchiveNodeMaxRecreateStateDepth
		} else {
			nodeConfig.Execution.RPC.MaxRecreateStateDepth = metachain.DefaultNonArchiveNodeMaxRecreateStateDepth
		}
	}
	...
	if nodeConfig.Node.Staker.OnlyCreateWalletContract {
		...
		l1Reader, err := headerreader.New(ctx, l1Client, func() *headerreader.Config { return &liveNodeConfig.Get().Node.ParentChainReader }, METASys)
		if err != nil {
	...
	// Check that node is compatible with on-chain WASM module root on startup and before any METAOS upgrades take effect to prevent divergences
	...
	METADb, err := stack.OpenDatabase("metachaindata", 0, 0, "", false)
	deferFuncs = append(deferFuncs, func() { closeDb(METADb, "METADb") })
	if err != nil {
	...
	if l2BlockChain.Config().metachainChainParams.DataAvailabilityCommittee && !nodeConfig.Node.DataAvailability.Enable {
		flag.Usage()
	...
	if err == nil && nodeConfig.Init.ResetToMessage > 0 {
		err = currentNode.TxStreamer.ReorgTo(METAutil.MessageIndex(nodeConfig.Init.ResetToMessage))
		if err != nil {
...
type NodeConfig struct {
	...
	Node          METAnode.Config                  `koanf:"node" reload:"hot"`
...
func NodeConfigAddOptions(f *flag.FlagSet) {
	...
	METAnode.ConfigAddOptions("node", f, true, true)
...
func applyChainParameters(ctx context.Context, k *koanf.Koanf, chainId uint64, chainName string, l2ChainInfoFiles []string, l2ChainInfoJson string, l2ChainInfoIpfsUrl string, l2ChainInfoIpfsDownloadPath string) error {
	...
	var parentChainIsmetachain bool
	if chainInfo.ParentChainIsmetachain != nil {
		parentChainIsmetachain = *chainInfo.ParentChainIsmetachain
	} else {
		log.Warn("Chain info field parent-chain-is-metachain is missing, in the future this will be required", "chainId", chainInfo.ChainConfig.ChainID, "parentChainId", chainInfo.ParentChainId)
		_, err := chaininfo.ProcessChainInfo(chainInfo.ParentChainId, "", combinedL2ChainInfoFiles, "")
		if err == nil {
			parentChainIsmetachain = true
		}
	}
	...
	} else if chainInfo.ChainConfig.metachainChainParams.DataAvailabilityCommittee {
		chainDefaults["node.data-availability.enable"] = true
	}
	if !chainInfo.HasGenesisState {
		chainDefaults["init.empty"] = true
	}
	if parentChainIsmetachain {
		...
		// metachain chains produce blocks more quickly, so the inbox reader should read more blocks at once.
...
func (f *NodeConfigFetcher) Get() *METAnode.Config {
