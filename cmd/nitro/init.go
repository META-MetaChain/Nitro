...
import (
	...
	"github.com/META-MetaChain/nitro/METAnode/dataposter/storage"
	...
	"github.com/META-MetaChain/nitro/cmd/ipfshelper"
...
func validateBlockChain(blockChain *core.BlockChain, chainConfig *params.ChainConfig) error {
	...
	currentMETAosState, err := METAosState.OpenSystemMETAosState(statedb, nil, true)
	if err != nil {
		return err
	}
	chainId, err := currentMETAosState.ChainId()
	if err != nil {
		return err
	}
	if chainId.Cmp(chainConfig.ChainID) != 0 {
		return fmt.Errorf("attempted to launch node with chain ID %v on METAOS state with chain ID %v", chainConfig.ChainID, chainId)
	}
	oldSerializedConfig, err := currentMETAosState.ChainConfig()
	if err != nil {
...
func findImportantRoots(ctx context.Context, chainDb ethdb.Database, stack *node.Node, nodeConfig *NodeConfig, cacheConfig *core.CacheConfig, l1Client METAutil.L1Interface, rollupAddrs chaininfo.RollupAddresses) ([]common.Hash, error) {
	...
	METADb, err := stack.OpenDatabase("metachaindata", 0, 0, "", true)
	if err != nil {
		return nil, err
	}
	defer func() {
		err := METADb.Close()
		if err != nil {
			log.Warn("failed to close metachain database after finding pruning targets", "err", err)
		}
	}()
	roots := importantRoots{
		chainDb: chainDb,
	}
	genesisNum := chainConfig.metachainChainParams.GenesisBlockNum
	...
	if initConfig.Prune == "validator" {
		...
		validatorDb := rawdb.NewTable(METADb, storage.BlockValidatorPrefix)
	...
	if l1Client != nil {
		...
		tracker, err := METAnode.NewInboxTracker(METADb, nil, nil)
		if err != nil {
			return nil, err
		}
		...
		for {
			...
			if meta.ParentChainBlock <= l1BlockNum {
				signedBlockNum := METAutil.MessageCountToBlockNumber(meta.MessageCount, genesisNum)
...
func pruneChainDb(ctx context.Context, chainDb ethdb.Database, stack *node.Node, nodeConfig *NodeConfig, cacheConfig *core.CacheConfig, l1Client METAutil.L1Interface, rollupAddrs chaininfo.RollupAddresses) error {
...
func openInitializeChainDb(ctx context.Context, stack *node.Node, config *NodeConfig, chainId *big.Int, cacheConfig *core.CacheConfig, l1Client METAutil.L1Interface, rollupAddrs chaininfo.RollupAddresses) (ethdb.Database, *core.BlockChain, error) {
	...
	if initDataReader == nil {
		...
		genesisBlockNr := chainConfig.metachainChainParams.GenesisBlockNum
		...
		var parsedInitMessage *METAostypes.ParsedInitMessage
		if config.Node.ParentChainReader.Enable {
			delayedBridge, err := METAnode.NewDelayedBridge(l1Client, rollupAddrs.Bridge, rollupAddrs.DeployedAt)
			if err != nil {
			...
			var initMessage *METAostypes.L1IncomingMessage
			for _, msg := range delayedMessages {
				if msg.Message.Header.Kind == METAostypes.L1MessageType_Initialize {
			...
			if parsedInitMessage.ChainConfig != nil {
				if err := parsedInitMessage.ChainConfig.CheckCompatible(chainConfig, chainConfig.metachainChainParams.GenesisBlockNum, 0); err != nil {
			...
			parsedInitMessage = &METAostypes.ParsedInitMessage{
...
func testUpdateTxIndex(chainDb ethdb.Database, chainConfig *params.ChainConfig, globalWg *sync.WaitGroup) {
	lastBlock := chainConfig.metachainChainParams.GenesisBlockNum
	if lastBlock == 0 {
