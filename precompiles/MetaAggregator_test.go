...
func TestMETAAggregatorBatchPosters(t *testing.T) {
	...
	bps, err := METAAggregator{}.GetBatchPosters(context, evm)
	Require(t, err)
	if len(bps) != 1 {
		Fail(t)
	}
	...
	Require(t, METADebug{}.BecomeChainOwner(context, evm))
	Require(t, METAAggregator{}.AddBatchPoster(context, evm, addr))
