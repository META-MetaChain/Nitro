...
func (con METADebug) Events(c ctx, evm mech, paid huge, flag bool, value bytes32) (addr, huge, error) {
...
func (con METADebug) EventsView(c ctx, evm mech) error {
	_, _, err := con.Events(c, evm, common.Big0, true, bytes32{})
	return err
}

func (con METADebug) CustomRevert(c ctx, number uint64) error {
	return con.CustomError(number, "This spider family wards off bugs: /\\oo/\\ //\\(oo)/\\ /\\oo/\\", true)
}

// Caller becomes a chain owner
func (con METADebug) BecomeChainOwner(c ctx, evm mech) error {
	return c.State.ChainOwners().Add(c.caller)
}

func (con METADebug) LegacyError(c ctx) error {
