...
type DebugPrecompile struct {
	precompile METAosPrecompile
}

// create a debug-only precompile wrapper
func debugOnly(address addr, impl METAosPrecompile) (addr, METAosPrecompile) {
...
func ownerOnly(address addr, impl METAosPrecompile, emit func(mech, bytes4, addr, []byte) error) (addr, METAosPrecompile) {
...
func (wrapper *OwnerPrecompile) Call(
	...
	state, err := METAosState.OpenMETAosState(evm.StateDB, burner)
	if err != nil {
	...
	version := METAosState.METAOSVersion(evm.StateDB)
