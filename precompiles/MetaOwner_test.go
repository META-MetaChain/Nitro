...
func TestMETAOwner(t *testing.T) {
	...
	tracer := util.NewTracingInfo(evm, testhelpers.RandomAddress(), types.METAosAddress, util.TracingDuringEVM)
	state, err := METAosState.OpenMETAosState(evm.StateDB, burn.NewSystemBurner(tracer, false))
	Require(t, err)
...
func TestMETAOwnerSetChainConfig(t *testing.T) {
	...
	chainConfig.metachainChainParams.AllowDebugPrecompiles = false
	...
	chainConfig.metachainChainParams.AllowDebugPrecompiles = true
	serializedChainConfig, err = json.Marshal(chainConfig)
	Require(t, err)
	err = prec.SetChainConfig(callCtx, evm, serializedChainConfig)
	Require(t, err)
...
func TestMETAInfraFeeAccount(t *testing.T) {
	version0 := uint64(0)
	...
	Require(t, err)
	err = prec.SetInfraFeeAccount(callCtx, evm, newAddr) // this should be a no-op (because METAOS version 0)
	Require(t, err)
