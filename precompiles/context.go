...
type Context struct {
	...
	gasSupplied uint64
	gasLeft     uint64
	txProcessor *METAos.TxProcessor
...
func testContext(caller addr, evm mech) *Context {
	tracingInfo := util.NewTracingInfo(evm, common.Address{}, types.METAosAddress, util.TracingDuringEVM)
	ctx := &Context{
		caller:      caller,
	...
	state, err := METAosState.OpenMETAosState(evm.StateDB, burn.NewSystemBurner(tracingInfo, false))
	if err != nil {
		log.Crit("unable to open METAos state", "error", err)
	}
	ctx.State = state
	var ok bool
	ctx.txProcessor, ok = evm.ProcessingHook.(*METAos.TxProcessor)
	if !ok {
