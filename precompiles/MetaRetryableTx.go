...
func (con METARetryableTx) oldNotFoundError(c ctx) error {
	if c.State.METAOSVersion() >= 3 {
...
func (con METARetryableTx) Redeem(c ctx, evm mech, ticketId bytes32) (bytes32, error) {
	...
	writeBytes := METAmath.WordsForBytes(byteCount)
	...
	return retryTxHash, c.State.L2PricingState().AddToGasPool(METAmath.SaturatingCast(gasToDonate))
}

// GetLifetime gets the default lifetime period a retryable has at creation
func (con METARetryableTx) GetLifetime(c ctx, evm mech) (huge, error) {
	return big.NewInt(retryables.RetryableLifetimeSeconds), nil
}

// GetTimeout gets the timestamp for when ticket will expire
func (con METARetryableTx) GetTimeout(c ctx, evm mech, ticketId bytes32) (huge, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time)
	if err != nil {
		return nil, err
...
func (con METARetryableTx) Keepalive(c ctx, evm mech, ticketId bytes32) (huge, error) {
	...
	updateCost := METAmath.WordsForBytes(nbytes) * params.SstoreSetGas / 100
...
func (con METARetryableTx) GetBeneficiary(c ctx, evm mech, ticketId bytes32) (addr, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time)
	if err != nil {
		return addr{}, err
...
func (con METARetryableTx) Cancel(c ctx, evm mech, ticketId bytes32) error {
	if c.txProcessor.CurrentRetryable != nil && ticketId == *c.txProcessor.CurrentRetryable {
...
func (con METARetryableTx) GetCurrentRedeemer(c ctx, evm mech) (common.Address, error) {
...
func (con METARetryableTx) SubmitRetryable(
