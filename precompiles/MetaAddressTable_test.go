...
func TestMETAAddressTableInit(t *testing.T) {
	evm := newMockEVMForTesting()
	atab := METAAddressTable{}
...
func newMockEVMForTestingWithVersionAndRunMode(version *uint64, runMode core.MessageRunMode) *vm.EVM {
	...
	evm.ProcessingHook = METAos.NewTxProcessor(evm, &core.Message{TxRunMode: runMode})
	return evm
}
...
func newMockEVMForTestingWithVersion(version *uint64) *vm.EVM {
	...
	if version != nil {
		chainConfig.metachainChainParams.InitialMETAOSVersion = *version
	}
	...
	evm.ProcessingHook = &METAos.TxProcessor{}
	return evm
}
