...
import (
	...
	"github.com/META-MetaChain/nitro/util/merkletree"
)

// METASys provides system-level functionality for interacting with L1 and understanding the call stack.
type METASys struct {
	Address                 addr // 0x64
...
// METABlockNumber gets the current L2 block number
func (con *METASys) METABlockNumber(c ctx, evm mech) (huge, error) {
	return evm.Context.BlockNumber, nil
}

// METABlockHash gets the L2 block hash, if sufficiently recent
func (con *METASys) METABlockHash(c ctx, evm mech, METABlockNumber *big.Int) (bytes32, error) {
	if !METABlockNumber.IsUint64() {
		if c.State.METAOSVersion() >= 11 {
			return bytes32{}, con.InvalidBlockNumberError(METABlockNumber, evm.Context.BlockNumber)
		}
		return bytes32{}, errors.New("invalid block number")
	}
	requestedBlockNum := METABlockNumber.Uint64()

	currentNumber := evm.Context.BlockNumber.Uint64()
	if requestedBlockNum >= currentNumber || requestedBlockNum+256 < currentNumber {
		if c.State.METAOSVersion() >= 11 {
			return common.Hash{}, con.InvalidBlockNumberError(METABlockNumber, evm.Context.BlockNumber)
		}
		return common.Hash{}, errors.New("invalid block number for METABlockHAsh")
	}

	return evm.Context.GetHash(requestedBlockNum), nil
}

// METAChainID gets the rollup's unique chain identifier
func (con *METASys) METAChainID(c ctx, evm mech) (huge, error) {
	return evm.ChainConfig().ChainID, nil
}

// METAOSVersion gets the current METAOS version
func (con *METASys) METAOSVersion(c ctx, evm mech) (huge, error) {
	version := new(big.Int).SetUint64(55 + c.State.METAOSVersion()) // Nitro starts at version 56
	return version, nil
}

// GetStorageGasAvailable returns 0 since Nitro has no concept of storage gas
func (con *METASys) GetStorageGasAvailable(c ctx, evm mech) (huge, error) {
	return big.NewInt(0), nil
}

// IsTopLevelCall checks if the call is top-level (deprecated)
func (con *METASys) IsTopLevelCall(c ctx, evm mech) (bool, error) {
	return evm.Depth() <= 2, nil
}

// MapL1SenderContractAddressToL2Alias gets the contract's L2 alias
func (con *METASys) MapL1SenderContractAddressToL2Alias(c ctx, sender addr, dest addr) (addr, error) {
	return util.RemapL1Address(sender), nil
}

// WasMyCallersAddressAliased checks if the caller's caller was aliased
func (con *METASys) WasMyCallersAddressAliased(c ctx, evm mech) (bool, error) {
...
func (con *METASys) MyCallersAddressWithoutAliasing(c ctx, evm mech) (addr, error) {
...
func (con *METASys) SendTxToL1(c ctx, evm mech, value huge, destination addr, calldataForL1 []byte) (huge, error) {
	l1BlockNum, err := c.txProcessor.L1BlockNumber(vm.BlockContext{})
	if err != nil {
		return nil, err
	}
	bigL1BlockNum := METAmath.UintToBig(l1BlockNum)

	METAosState := c.State
	var t big.Int
	t.SetUint64(evm.Context.Time)
	sendHash, err := METAosState.KeccakHash(
		c.caller.Bytes(),
		destination.Bytes(),
		METAmath.U256Bytes(evm.Context.BlockNumber),
		METAmath.U256Bytes(bigL1BlockNum),
		METAmath.U256Bytes(&t),
	...
	merkleAcc := METAosState.SendMerkleAccumulator()
...
func (con METASys) SendMerkleTreeState(c ctx, evm mech) (huge, bytes32, []bytes32, error) {
	if c.caller != (addr{}) {
...
func (con METASys) WithdrawEth(c ctx, evm mech, value huge, destination addr) (huge, error) {
	return con.SendTxToL1(c, evm, value, destination, []byte{})
}

func (con METASys) isTopLevel(c ctx, evm mech) bool {
	depth := evm.Depth()
