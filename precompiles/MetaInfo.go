...
// METAInfo povides the ability to lookup basic info about accounts and contracts.
type METAInfo struct {
	Address addr // 0x65
}

// GetBalance retrieves an account's balance
func (con METAInfo) GetBalance(c ctx, evm mech, account addr) (huge, error) {
...
func (con METAInfo) GetCode(c ctx, evm mech, account addr) ([]byte, error) {
	if err := c.Burn(params.ColdSloadCostEIP2929); err != nil {
		return nil, err
	}
	code := evm.StateDB.GetCode(account)
	if err := c.Burn(params.CopyGas * METAmath.WordsForBytes(uint64(len(code)))); err != nil {
		return nil, err
	}
	return code, nil
}
