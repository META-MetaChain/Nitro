...
func TestEvents(t *testing.T) {
	...
	burnedToStorage := storage.StorageReadCost                      // the METAOS version check costs a read
	burnedToArgs := METAmath.WordsForBytes(32+32) * params.CopyGas   // bool and a bytes32
	burnedToResult := METAmath.WordsForBytes(32+32) * params.CopyGas // addr and a huge
	...
	METADebugInfo, cerr := templates.NewMETADebug(common.Address{}, nil)
	basic, berr := METADebugInfo.ParseBasic(*logs[0])
	mixed, merr := METADebugInfo.ParseMixed(*logs[1])
...
func TestEventCosts(t *testing.T) {
	...
	//nolint:errcheck
	impl := contract.Precompile().implementer.Interface().(*METADebug)
