...
import (
	...
	templates "github.com/META-MetaChain/nitro/solgen/go/precompilesgen"
...
type METAosPrecompile interface {
...
func MakePrecompile(metadata *bind.MetaData, implementer interface{}) (addr, *Precompile) {
	...
	for _, event := range source.Events {
		...
		emit := func(args []reflect.Value) []reflect.Value {
			...
			version := METAosState.METAOSVersion(state)
...
func Precompiles() map[addr]METAosPrecompile {
	...
	contracts := make(map[addr]METAosPrecompile)

	insert := func(address addr, impl METAosPrecompile) *Precompile {
		contracts[address] = impl
		return impl.Precompile()
	}

	insert(MakePrecompile(templates.METAInfoMetaData, &METAInfo{Address: hex("65")}))
	insert(MakePrecompile(templates.METAAddressTableMetaData, &METAAddressTable{Address: hex("66")}))
	insert(MakePrecompile(templates.METABLSMetaData, &METABLS{Address: hex("67")}))
	insert(MakePrecompile(templates.METAFunctionTableMetaData, &METAFunctionTable{Address: hex("68")}))
	insert(MakePrecompile(templates.METAosTestMetaData, &METAosTest{Address: hex("69")}))
	METAGasInfo := insert(MakePrecompile(templates.METAGasInfoMetaData, &METAGasInfo{Address: hex("6c")}))
	METAGasInfo.methodsByName["GetL1FeesAvailable"].METAosVersion = 10
	METAGasInfo.methodsByName["GetL1RewardRate"].METAosVersion = 11
	METAGasInfo.methodsByName["GetL1RewardRecipient"].METAosVersion = 11
	insert(MakePrecompile(templates.METAAggregatorMetaData, &METAAggregator{Address: hex("6d")}))
	insert(MakePrecompile(templates.METAStatisticsMetaData, &METAStatistics{Address: hex("6f")}))
	...
	METAOwnerPublic := insert(MakePrecompile(templates.METAOwnerPublicMetaData, &METAOwnerPublic{Address: hex("6b")}))
	METAOwnerPublic.methodsByName["GetInfraFeeAccount"].METAosVersion = 5
	METAOwnerPublic.methodsByName["RectifyChainOwner"].METAosVersion = 11
	METAOwnerPublic.methodsByName["GetBrotliCompressionLevel"].METAosVersion = 20

	METARetryableImpl := &METARetryableTx{Address: types.METARetryableTxAddress}
	METARetryable := insert(MakePrecompile(templates.METARetryableTxMetaData, METARetryableImpl))
	METAos.METARetryableTxAddress = METARetryable.address
	METAos.RedeemScheduledEventID = METARetryable.events["RedeemScheduled"].template.ID
	METAos.EmitReedeemScheduledEvent = func(
		evm mech, gas, nonce uint64, ticketId, retryTxHash bytes32,
		donor addr, maxRefund *big.Int, submissionFeeRefund *big.Int,
	) error {
		zero := common.Big0
		context := eventCtx(METARetryableImpl.RedeemScheduledGasCost(hash{}, hash{}, 0, 0, addr{}, zero, zero))
		return METARetryableImpl.RedeemScheduled(
			context, evm, ticketId, retryTxHash, nonce, gas, donor, maxRefund, submissionFeeRefund,
		)
	}
	METAos.EmitTicketCreatedEvent = func(evm mech, ticketId bytes32) error {
		context := eventCtx(METARetryableImpl.TicketCreatedGasCost(hash{}))
		return METARetryableImpl.TicketCreated(context, evm, ticketId)
	}

	METASys := insert(MakePrecompile(templates.METASysMetaData, &METASys{Address: types.METASysAddress}))
	METAos.METASysAddress = METASys.address
	METAos.L2ToL1TransactionEventID = METASys.events["L2ToL1Transaction"].template.ID
	METAos.L2ToL1TxEventID = METASys.events["L2ToL1Tx"].template.ID

	METAOwnerImpl := &METAOwner{Address: hex("70")}
	emitOwnerActs := func(evm mech, method bytes4, owner addr, data []byte) error {
		context := eventCtx(METAOwnerImpl.OwnerActsGasCost(method, owner, data))
		return METAOwnerImpl.OwnerActs(context, evm, method, owner, data)
	}
	_, METAOwner := MakePrecompile(templates.METAOwnerMetaData, METAOwnerImpl)
	METAOwner.methodsByName["GetInfraFeeAccount"].METAosVersion = 5
	METAOwner.methodsByName["SetInfraFeeAccount"].METAosVersion = 5
	METAOwner.methodsByName["ReleaseL1PricerSurplusFunds"].METAosVersion = 10
	METAOwner.methodsByName["SetChainConfig"].METAosVersion = 11
	METAOwner.methodsByName["SetBrotliCompressionLevel"].METAosVersion = 20

	insert(ownerOnly(METAOwnerImpl.Address, METAOwner, emitOwnerActs))
	insert(debugOnly(MakePrecompile(templates.METADebugMetaData, &METADebug{Address: hex("ff")})))

	METAosActs := insert(MakePrecompile(templates.METAosActsMetaData, &METAosActs{Address: types.METAosAddress}))
	METAos.InternalTxStartBlockMethodID = METAosActs.GetMethodID("StartBlock")
	METAos.InternalTxBatchPostingReportMethodID = METAosActs.GetMethodID("BatchPostingReport")
...
func (p *Precompile) Call(
	...
	METAosVersion := METAosState.METAOSVersion(evm.StateDB)

	if METAosVersion < p.METAosVersion {
	...
	if len(input) < 4 {
		// METAOS precompiles always have canonical method selectors
		return nil, 0, vm.ErrExecutionReverted
	}
	id := *(*[4]byte)(input)
	method, ok := p.methods[id]
	if !ok || METAosVersion < method.METAosVersion {
	...
	argsCost := params.CopyGas * METAmath.WordsForBytes(uint64(len(input)-4))
	...
	if method.purity != pure {
		// impure methods may need the METAOS state, so open & update the call context now
		state, err := METAosState.OpenMETAosState(evm.StateDB, callerCtx)
		if err != nil {
			return nil, 0, err
		}
	...
	if !reflectResult[resultCount].IsNil() {
		...
		if isSolErr {
			resultCost := params.CopyGas * METAmath.WordsForBytes(uint64(len(solErr.data)))
		...
		// nolint:errorlint
		if METAosVersion >= 11 || errRet == vm.ErrExecutionReverted {
			return nil, callerCtx.gasLeft, vm.ErrExecutionReverted
		}
	...
	resultCost := params.CopyGas * METAmath.WordsForBytes(uint64(len(encoded)))
	if err := callerCtx.Burn(resultCost); err != nil {
		// user cannot afford the result data returned
		return nil, 0, vm.ErrExecutionReverted
	}
