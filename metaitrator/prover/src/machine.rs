...
use crate::{
    ...
    value::{METAValueType, FunctionType, IntegerValType, ProgramCounter, Value},
    wavm::{
...
impl Table {
    fn serialize_for_proof(&self) -> Result<Vec<u8>> {
        let mut data = vec![METAValueType::try_from(self.ty.element_type)?.serialize()];
    ...
    fn hash(&self) -> Result<Bytes32> {
        let mut h = Keccak256::new();
        h.update("Table:");
        h.update([METAValueType::try_from(self.ty.element_type)?.serialize()]);
...
impl Machine {
    ...
    pub fn from_binaries(
        ...
        if let Some(&f) = main_module.exports.get("run").filter(|_| runtime_support) {
            ...
            // That's also handled by the Go compiler. Go 1.17.5 in the compilation of the METAitrator go test case
        ...
        for _ in 0..n {
            ...
            match inst.opcode {
                ...
                Opcode::Reinterpret(dest, source) => {
                    let val = match self.value_stack.pop() {
                        Some(Value::I32(x)) if source == METAValueType::I32 => {
                            assert_eq!(dest, METAValueType::F32, "Unsupported reinterpret");
                            Value::F32(f32::from_bits(x))
                        }
                        Some(Value::I64(x)) if source == METAValueType::I64 => {
                            assert_eq!(dest, METAValueType::F64, "Unsupported reinterpret");
                            Value::F64(f64::from_bits(x))
                        }
                        Some(Value::F32(x)) if source == METAValueType::F32 => {
                            assert_eq!(dest, METAValueType::I32, "Unsupported reinterpret");
                            Value::I32(x.to_bits())
                        }
                        Some(Value::F64(x)) if source == METAValueType::F64 => {
                            assert_eq!(dest, METAValueType::I64, "Unsupported reinterpret");
                            Value::I64(x.to_bits())
                        }
