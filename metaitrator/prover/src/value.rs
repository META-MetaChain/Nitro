...
impl TryFrom<Type> for METAValueType {
    type Error = eyre::Error;

    fn try_from(ty: Type) -> Result<METAValueType> {
        use Type::*;
        Ok(match ty {
            I32 => Self::I32,
            I64 => Self::I64,
...
impl From<FloatType> for METAValueType {
    fn from(ty: FloatType) -> METAValueType {
        match ty {
            FloatType::F32 => METAValueType::F32,
            FloatType::F64 => METAValueType::F64,
        }
    }
}
...
impl From<IntegerValType> for METAValueType {
    fn from(ty: IntegerValType) -> METAValueType {
        match ty {
            IntegerValType::I32 => METAValueType::I32,
            IntegerValType::I64 => METAValueType::I64,
        }
    }
}
...
impl Value {
    pub fn ty(self) -> METAValueType {
        match self {
            Value::I32(_) => METAValueType::I32,
            Value::I64(_) => METAValueType::I64,
            Value::F32(_) => METAValueType::F32,
            Value::F64(_) => METAValueType::F64,
            Value::RefNull => METAValueType::RefNull,
            Value::FuncRef(_) => METAValueType::FuncRef,
            Value::InternalRef(_) => METAValueType::InternalRef,
        }
    }
    ...
    pub fn default_of_type(ty: METAValueType) -> Value {
        match ty {
            METAValueType::I32 => Value::I32(0),
            METAValueType::I64 => Value::I64(0),
            METAValueType::F32 => Value::F32(0.),
            METAValueType::F64 => Value::F64(0.),
            METAValueType::RefNull | METAValueType::FuncRef | METAValueType::InternalRef => {
                Value::RefNull
            }
        }
    }
...
pub struct FunctionType {
    pub inputs: Vec<METAValueType>,
    pub outputs: Vec<METAValueType>,
}

impl FunctionType {
    pub fn new(inputs: Vec<METAValueType>, outputs: Vec<METAValueType>) -> FunctionType {
...
impl TryFrom<FuncType> for FunctionType {
    ...
    fn try_from(func: FuncType) -> Result<Self> {
        ...
        for input in func.params.iter() {
            inputs.push(METAValueType::try_from(*input)?)
        }
        for output in func.returns.iter() {
            outputs.push(METAValueType::try_from(*output)?)
        }
