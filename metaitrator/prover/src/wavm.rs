...
use crate::{
    ...
    host::InternalFunc,
    utils::Bytes32,
    value::{METAValueType, FunctionType, IntegerValType},
};
use digest::Digest;
...
pub enum Opcode {
    ...
    Reinterpret(METAValueType, METAValueType),
    ...
    InitFrame,
    /// Unconditional jump to an METAitrary point in code.
    METAitraryJump,
    /// Conditional jump to an METAitrary point in code.
    METAitraryJumpIf,
    ...
    pub fn repr(self) -> u16 {
        match self {
            ...
            Opcode::MemoryLoad { ty, bytes, signed } => match (ty, bytes, signed) {
                (METAValueType::I32, 4, false) => 0x28,
                (METAValueType::I64, 8, false) => 0x29,
                (METAValueType::F32, 4, false) => 0x2A,
                (METAValueType::F64, 8, false) => 0x2B,
                (METAValueType::I32, 1, true) => 0x2C,
                (METAValueType::I32, 1, false) => 0x2D,
                (METAValueType::I32, 2, true) => 0x2E,
                (METAValueType::I32, 2, false) => 0x2F,
                (METAValueType::I64, 1, true) => 0x30,
                (METAValueType::I64, 1, false) => 0x31,
                (METAValueType::I64, 2, true) => 0x32,
                (METAValueType::I64, 2, false) => 0x33,
                (METAValueType::I64, 4, true) => 0x34,
                (METAValueType::I64, 4, false) => 0x35,
                _ => panic!(
            ...
            Opcode::MemoryStore { ty, bytes } => match (ty, bytes) {
                (METAValueType::I32, 4) => 0x36,
                (METAValueType::I64, 8) => 0x37,
                (METAValueType::F32, 4) => 0x38,
                (METAValueType::F64, 8) => 0x39,
                (METAValueType::I32, 1) => 0x3A,
                (METAValueType::I32, 2) => 0x3B,
                (METAValueType::I64, 1) => 0x3C,
                (METAValueType::I64, 2) => 0x3D,
                (METAValueType::I64, 4) => 0x3E,
                _ => panic!(
            ...
            Opcode::Reinterpret(dest, source) => match (dest, source) {
                (METAValueType::I32, METAValueType::F32) => 0xBC,
                (METAValueType::I64, METAValueType::F64) => 0xBD,
                (METAValueType::F32, METAValueType::I32) => 0xBE,
                (METAValueType::F64, METAValueType::I64) => 0xBF,
            ...
            Opcode::METAitraryJump => 0x8003,
            Opcode::METAitraryJumpIf => 0x8004,
...
pub fn wasm_to_wavm(
    ...
    macro_rules! reinterpret {
        ($dest:ident, $source:ident) => {{
            let op = Opcode::Reinterpret(METAValueType::$dest, METAValueType::$source);
            out.push(Instruction::simple(op));
        }};
    }
    macro_rules! call {
    ...
    macro_rules! float {
        ...
        (@impl $func:expr) => {{
            ...
            for &arg in sig.inputs.iter().rev() {
                match arg {
                    METAValueType::I32 | METAValueType::I64 => {}
                    METAValueType::F32 => reinterpret!(I32, F32),
                    METAValueType::F64 => reinterpret!(I64, F64),
            ...
            match outputs.as_slice() {
                &[METAValueType::I32] => {}
                &[METAValueType::I64] => {}
                &[METAValueType::F32] => reinterpret!(F32, I32),
                &[METAValueType::F64] => reinterpret!(F64, I64),
    ...
    macro_rules! branch {
        ($kind:ident, $depth:expr) => {{
            ...
            if jump_op == Opcode::METAitraryJumpIf {
                stack -= 1;
            } else {
                assert_eq!(jump_op, Opcode::METAitraryJump, "unrecognized jump op");
            }
            ...
            } else if jump_op == Opcode::METAitraryJump {
    ...
    for op in code {
        ...
        match op {
            ...
            Br { relative_depth } => branch!(METAitraryJump, *relative_depth),
            BrIf { relative_depth } => branch!(METAitraryJumpIf, *relative_depth),
            BrTable { table } => {
                ...
                for (index, target) in table.targets().enumerate() {
                    ...
                    opcode!(METAitraryJumpIf, @pop 1);
                }

                // nothing matched: drop the index and jump to the default.
                opcode!(Drop, @pop 1);
                branch!(METAitraryJump, table.default());
                ...
                for (jump, branch) in subjumps {
                    ...
                    stack = start_stack;
                    opcode!(Drop, @pop 1);
                    branch!(METAitraryJump, branch);
                }
            }
            Return => branch!(METAitraryJump, scopes.len() - 1),
