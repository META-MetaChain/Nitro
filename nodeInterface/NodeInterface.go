...
func (n NodeInterface) NitroGenesisBlock(c ctx) (huge, error) {
	block := n.backend.ChainConfig().metachainChainParams.GenesisBlockNum
	return METAmath.UintToBig(block), nil
}

func (n NodeInterface) FindBatchContainingBlock(c ctx, evm mech, blockNum uint64) (uint64, error) {
	node, err := METANodeFromNodeInterfaceBackend(n.backend)
	if err != nil {
		return 0, err
	}
...
func (n NodeInterface) GetL1Confirmations(c ctx, evm mech, blockHash bytes32) (uint64, error) {
	...
	if latestL1Block < meta.ParentChainBlock || METAutil.BlockNumberToMessageCount(blockNum, genesis) > meta.MessageCount {
		return 0, nil
	}
...
func (n NodeInterface) EstimateRetryableTicket(
	...
	submitTx := &types.metachainSubmitRetryableTx{
		ChainId:          nil,
		RequestId:        hash{},
	...
	// metachainSubmitRetryableTx is unsigned so the following won't panic
	msg, err := core.TransactionToMessage(types.NewTx(submitTx), types.NewmetachainSigner(nil), nil)
	if err != nil {
		return err
	}
...
func (n NodeInterface) ConstructOutboxProof(c ctx, evm mech, size, leaf uint64) (bytes32, bytes32, []bytes32, error) {
	...
	balanced := size == METAmath.NextPowerOf2(size)/2
	treeLevels := int(METAmath.Log2ceil(size)) // the # of levels in the tree
	...
	search = func(lo, hi uint64, find []merkletree.LevelAndLeaf) {
		...
		if lo == hi {
			...
			for _, tx := range all {
				for _, log := range tx {
					if log.Address != types.METASysAddress {
						// log not produced by METAOS
						continue
					}
...
func (n NodeInterface) messageArgs(
	...
	args := metachain.TransactionArgs{
...
func (n NodeInterface) GasEstimateL1Component(
	...
	feeForL1 = METAmath.BigMulByBips(feeForL1, METAos.GasEstimationL1PricePadding)
	gasForL1 := METAmath.BigDiv(feeForL1, baseFee).Uint64()
...
func (n NodeInterface) GasEstimateComponents(
	...
	backend, ok := n.backend.(*metachain.APIBackend)
	if !ok {
	...
	totalRaw, err := metachain.EstimateGas(context, backend, args, block, nil, gasCap)
	if err != nil {
	...
	// Compute the fee paid for L1 in L2 terms
	gasForL1 := METAos.GetPosterGas(c.State, baseFee, core.MessageGasEstimationMode, feeForL1)

	return total, gasForL1, baseFee, l1BaseFeeEstimate, nil
}

func findBatchContainingBlock(node *METAnode.Node, genesis uint64, block uint64) (uint64, error) {
	if block <= genesis {
		return 0, fmt.Errorf("%wblock %v is part of genesis", blockInGenesis, block)
	}
	pos := METAutil.BlockNumberToMessageCount(block, genesis) - 1
	...
	latestBlock := METAutil.MessageCountToBlockNumber(latestCount, genesis)
...
func (n NodeInterface) L2BlockRangeForL1(c ctx, evm mech, l1BlockNum uint64) (uint64, uint64, error) {
	...
	genesis := n.backend.ChainConfig().metachainChainParams.GenesisBlockNum
	...
	firstL2BlockForL1 := func(target uint64) (uint64, error) {
		...
		for low < high {
			mid := METAmath.SaturatingUAdd(low, high) / 2
