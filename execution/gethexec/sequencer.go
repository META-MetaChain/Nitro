...
var (
	sequencerBacklogGauge                   = metrics.NewRegisteredGauge("META/sequencer/backlog", nil)
	nonceCacheHitCounter                    = metrics.NewRegisteredCounter("META/sequencer/noncecache/hit", nil)
	nonceCacheMissCounter                   = metrics.NewRegisteredCounter("META/sequencer/noncecache/miss", nil)
	nonceCacheRejectedCounter               = metrics.NewRegisteredCounter("META/sequencer/noncecache/rejected", nil)
	nonceCacheClearedCounter                = metrics.NewRegisteredCounter("META/sequencer/noncecache/cleared", nil)
	nonceFailureCacheSizeGauge              = metrics.NewRegisteredGauge("META/sequencer/noncefailurecache/size", nil)
	nonceFailureCacheOverflowCounter        = metrics.NewRegisteredGauge("META/sequencer/noncefailurecache/overflow", nil)
	blockCreationTimer                      = metrics.NewRegisteredTimer("META/sequencer/block/creation", nil)
	successfulBlocksCounter                 = metrics.NewRegisteredCounter("META/sequencer/block/successful", nil)
	conditionalTxRejectedBySequencerCounter = metrics.NewRegisteredCounter("META/sequencer/condtionaltx/rejected", nil)
	conditionalTxAcceptedBySequencerCounter = metrics.NewRegisteredCounter("META/sequencer/condtionaltx/accepted", nil)
)
...
func (s *Sequencer) PublishTransaction(parentCtx context.Context, tx *types.Transaction, options *metachain_types.ConditionalOptions) error {
	...
	if tx.Type() >= types.metachainDepositTxType || tx.Type() == types.BlobTxType {
		// Should be unreachable for metachain types due to UnmarshalBinary not accepting metachain internal txs
		// and we want to disallow BlobTxType since metachain doesn't support EIP-4844 txs yet.
...
func (s *Sequencer) preTxFilter(_ *params.ChainConfig, header *types.Header, statedb *state.StateDB, _ *METAosState.METAosState, tx *types.Transaction, options *metachain_types.ConditionalOptions, sender common.Address, l1Info *METAos.L1Info) error {
...
func (s *Sequencer) postTxFilter(header *types.Header, _ *METAosState.METAosState, tx *types.Transaction, sender common.Address, dataGas uint64, result *core.ExecutionResult) error {
	if result.Err != nil && result.UsedGas > dataGas && result.UsedGas-dataGas <= s.config().MaxRevertGasReject {
		return metachain.NewRevertReason(result)
	}
	newNonce := tx.Nonce() + 1
...
func (s *Sequencer) makeSequencingHooks() *METAos.SequencingHooks {
	return &METAos.SequencingHooks{
...
func (s *Sequencer) precheckNonces(queueItems []txQueueItem) []txQueueItem {
	...
	nextHeaderNumber := METAmath.BigAdd(latestHeader.Number, common.Big1)
...
func (s *Sequencer) createBlock(ctx context.Context) (returnValue bool) {
	...
	hooks.ConditionalOptionsForTx = make([]*metachain_types.ConditionalOptions, len(queueItems))
	...
	header := &METAostypes.L1IncomingMessageHeader{
		Kind:        METAostypes.L1MessageType_L2Message,
...
func (s *Sequencer) updateLatestParentChainBlock(header *types.Header) {
	...
	l1BlockNumber := METAutil.ParentHeaderToL1BlockNumber(header)
