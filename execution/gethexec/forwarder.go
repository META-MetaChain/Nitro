...
func (f *TxForwarder) PublishTransaction(inctx context.Context, tx *types.Transaction, options *metachain_types.ConditionalOptions) error {
	if !f.enabled.Load() {
...
func (f *TxForwarder) CheckHealth(inctx context.Context) error {
	...
	if time.Since(f.healthChecked) > cacheUpstreamHealth {
		...
		defer cancelFunc()
		f.healthErr = f.rpcClients[0].CallContext(ctx, nil, "META_checkPublisherHealth")
...
func (f *TxDropper) PublishTransaction(ctx context.Context, tx *types.Transaction, options *metachain_types.ConditionalOptions) error {
	return txDropperErr
}
...
func (f *RedisTxForwarder) PublishTransaction(ctx context.Context, tx *types.Transaction, options *metachain_types.ConditionalOptions) error {
