...
type ExecutionEngine struct {
	...
	resequenceChan    chan []*METAostypes.MessageWithMetadata
...
func NewExecutionEngine(bc *core.BlockChain) (*ExecutionEngine, error) {
	return &ExecutionEngine{
		bc:               bc,
		resequenceChan:   make(chan []*METAostypes.MessageWithMetadata),
...
func (s *ExecutionEngine) Reorg(count METAutil.MessageIndex, newMessages []METAostypes.MessageWithMetadata, oldMessages []*METAostypes.MessageWithMetadata) error {
	if count == 0 {
	...
	for i := range newMessages {
		err := s.digestMessageWithBlockMutex(count+METAutil.MessageIndex(i), &newMessages[i])
		if err != nil {
			return err
		}
	}
...
func (s *ExecutionEngine) HeadMessageNumber() (METAutil.MessageIndex, error) {
...
func (s *ExecutionEngine) HeadMessageNumberSync(t *testing.T) (METAutil.MessageIndex, error) {
	s.createBlocksMutex.Lock()
...
func messageFromTxes(header *METAostypes.L1IncomingMessageHeader, txes types.Transactions, txErrors []error) (*METAostypes.L1IncomingMessage, error) {
	var l2Message []byte
	if len(txes) == 1 && txErrors[0] == nil {
		...
		l2Message = append(l2Message, METAos.L2MessageKind_SignedTx)
		l2Message = append(l2Message, txBytes...)
	} else {
		l2Message = append(l2Message, METAos.L2MessageKind_Batch)
		sizeBuf := make([]byte, 8)
		for i, tx := range txes {
			if txErrors[i] != nil {
				continue
	...
	return &METAostypes.L1IncomingMessage{
		Header: header,
		L2msg:  l2Message,
	}, nil
}
...
func (s *ExecutionEngine) resequenceReorgedMessages(messages []*METAostypes.MessageWithMetadata) {
	if !s.reorgSequencing {
		return
	}
	...
	for _, msg := range messages {
		...
		if header.Kind != METAostypes.L1MessageType_L2Message || header.Poster != l1pricing.BatchPosterAddress {
		...
		txes, err := METAos.ParseL2Transactions(msg.Message, s.bc.Config().ChainID, nil)
		if err != nil {
			log.Warn("failed to parse sequencer message found from reorg", "err", err)
			continue
		}
		hooks := METAos.NoopSequencingHooks()
...
func (s *ExecutionEngine) sequencerWrapper(sequencerFunc func() (*types.Block, error)) (*types.Block, error) {
	...
	for {
		...
		if !s.bc.Config().metachainChainParams.AllowDebugPrecompiles {
...
func (s *ExecutionEngine) SequenceTransactions(header *METAostypes.L1IncomingMessageHeader, txes types.Transactions, hooks *METAos.SequencingHooks) (*types.Block, error) {
...
func (s *ExecutionEngine) sequenceTransactionsWithBlockMutex(header *METAostypes.L1IncomingMessageHeader, txes types.Transactions, hooks *METAos.SequencingHooks) (*types.Block, error) {
	lastBlockHeader, err := s.getCurrentHeader()
	if err != nil {
		return nil, err
	}
	...
	startTime := time.Now()
	block, receipts, err := METAos.ProduceBlockAdvanced(
		header,
		txes,
		delayedMessagesRead,
		lastBlockHeader,
	...
	msgWithMeta := METAostypes.MessageWithMetadata{
		Message:             msg,
...
func (s *ExecutionEngine) SequenceDelayedMessage(message *METAostypes.L1IncomingMessage, delayedSeqNum uint64) error {
...
func (s *ExecutionEngine) sequenceDelayedMessageWithBlockMutex(message *METAostypes.L1IncomingMessage, delayedSeqNum uint64) (*types.Block, error) {
	currentHeader, err := s.getCurrentHeader()
	if err != nil {
		return nil, err
	}
	...
	messageWithMeta := METAostypes.MessageWithMetadata{
		Message:             message,
...
func (s *ExecutionEngine) GetGenesisBlockNumber() uint64 {
	return s.bc.Config().metachainChainParams.GenesisBlockNum
}

func (s *ExecutionEngine) BlockNumberToMessageIndex(blockNum uint64) (METAutil.MessageIndex, error) {
	genesis := s.GetGenesisBlockNumber()
	if blockNum < genesis {
		return 0, fmt.Errorf("blockNum %d < genesis %d", blockNum, genesis)
	}
	return METAutil.MessageIndex(blockNum - genesis), nil
}

func (s *ExecutionEngine) MessageIndexToBlockNumber(messageNum METAutil.MessageIndex) uint64 {
	return uint64(messageNum) + s.GetGenesisBlockNumber()
}

// must hold createBlockMutex
func (s *ExecutionEngine) createBlockFromNextMessage(msg *METAostypes.MessageWithMetadata) (*types.Block, *state.StateDB, types.Receipts, error) {
	...
	block, receipts, err := METAos.ProduceBlock(
		msg.Message,
...
func (s *ExecutionEngine) ResultAtPos(pos METAutil.MessageIndex) (*execution.MessageResult, error) {
	return s.resultFromHeader(s.bc.GetHeaderByNumber(s.MessageIndexToBlockNumber(pos)))
}

func (s *ExecutionEngine) DigestMessage(num METAutil.MessageIndex, msg *METAostypes.MessageWithMetadata) error {
...
func (s *ExecutionEngine) digestMessageWithBlockMutex(num METAutil.MessageIndex, msg *METAostypes.MessageWithMetadata) error {
	currentHeader, err := s.getCurrentHeader()
	if err != nil {
		return err
	}
	...
	if time.Now().After(s.nextScheduledVersionCheck) {
		...
		METAState, err := METAosState.OpenSystemMETAosState(statedb, nil, true)
		if err != nil {
			return err
		}
		version, timestampInt, err := METAState.GetScheduledUpgrade()
		if err != nil {
			return err
		}
		...
		maxSupportedVersion := params.metachainDevTestChainConfig().metachainChainParams.InitialMETAOSVersion
		logLevel := log.Warn
		...
		if version > maxSupportedVersion {
			logLevel(
				"you need to update your node to the latest version before this scheduled METAOS upgrade",
				"timeUntilUpgrade", timeUntilUpgrade,
				"upgradeScheduledFor", timestamp,
				"maxSupportedMETAosVersion", maxSupportedVersion,
				"pendingMETAosUpgradeVersion", version,
			)
		}
	}
