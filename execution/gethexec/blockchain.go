...
func WriteOrTestGenblock(chainDb ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, initMessage *METAostypes.ParsedInitMessage, accountsPerSync uint) error {
	...
	stateRoot, err := METAosState.InitializeMETAosInDatabase(chainDb, initData, chainConfig, initMessage, timestamp, accountsPerSync)
	if err != nil {
		return err
	}

	genBlock := METAosState.MakeGenesisBlock(prevHash, blockNumber, timestamp, stateRoot, chainConfig)
...
func WriteOrTestBlockChain(chainDb ethdb.Database, cacheConfig *core.CacheConfig, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, initMessage *METAostypes.ParsedInitMessage, txLookupLimit uint64, accountsPerSync uint) (*core.BlockChain, error) {
