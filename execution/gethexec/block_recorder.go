...
type BlockRecorder struct {
	recordingDatabase *metachain.RecordingDatabase
...
func NewBlockRecorder(config *metachain.RecordingDatabaseConfig, execEngine *ExecutionEngine, ethDb ethdb.Database) *BlockRecorder {
	recorder := &BlockRecorder{
		execEngine:        execEngine,
		recordingDatabase: metachain.NewRecordingDatabase(config, ethDb, execEngine.bc),
	}
...
func (r *BlockRecorder) RecordBlockCreation(
	...
	if prevHeader != nil {
		initialMETAosState, err := METAosState.OpenSystemMETAosState(recordingdb, nil, true)
		if err != nil {
			return nil, fmt.Errorf("error opening initial METAOS state: %w", err)
		}
		chainId, err := initialMETAosState.ChainId()
		if err != nil {
			return nil, fmt.Errorf("error getting chain ID from initial METAOS state: %w", err)
		}
		if chainId.Cmp(chainConfig.ChainID) != 0 {
			return nil, fmt.Errorf("unexpected chain ID %r in METAOS state, expected %r", chainId, chainConfig.ChainID)
		}
		genesisNum, err := initialMETAosState.GenesisBlockNum()
		if err != nil {
			return nil, fmt.Errorf("error getting genesis block number from initial METAOS state: %w", err)
		}
		_, err = initialMETAosState.ChainConfig()
		if err != nil {
			return nil, fmt.Errorf("error getting chain config from initial METAOS state: %w", err)
		}
		expectedNum := chainConfig.metachainChainParams.GenesisBlockNum
		if genesisNum != expectedNum {
			return nil, fmt.Errorf("unexpected genesis block number %v in METAOS state, expected %v", genesisNum, expectedNum)
		}
	}
...
func (r *BlockRecorder) MarkValid(pos METAutil.MessageIndex, resultHash common.Hash) {
	r.validHdrLock.Lock()
	defer r.validHdrLock.Unlock()
	if r.validHdrCandidate == nil {
		return
...
func (r *BlockRecorder) PrepareForRecord(ctx context.Context, start, end METAutil.MessageIndex) error {
