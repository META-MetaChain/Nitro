...
var (
	conditionalTxRejectedByTxPreCheckerCurrentStateCounter = metrics.NewRegisteredCounter("META/txprechecker/condtionaltx/currentstate/rejected", nil)
	conditionalTxAcceptedByTxPreCheckerCurrentStateCounter = metrics.NewRegisteredCounter("META/txprechecker/condtionaltx/currentstate/accepted", nil)
	conditionalTxRejectedByTxPreCheckerOldStateCounter     = metrics.NewRegisteredCounter("META/txprechecker/condtionaltx/oldstate/rejected", nil)
	conditionalTxAcceptedByTxPreCheckerOldStateCounter     = metrics.NewRegisteredCounter("META/txprechecker/condtionaltx/oldstate/accepted", nil)
)
...
func PreCheckTx(bc *core.BlockChain, chainConfig *params.ChainConfig, header *types.Header, statedb *state.StateDB, METAos *METAosState.METAosState, tx *types.Transaction, options *metachain_types.ConditionalOptions, config *TxPreCheckerConfig) error {
	...
	if config.Strictness < TxPreCheckerStrictnessLikelyCompatible {
		baseFee, err = METAos.L2PricingState().MinBaseFeeWei()
		if err != nil {
			return err
		}
	}
	if METAmath.BigLessThan(tx.GasFeeCap(), baseFee) {
	...
	intrinsic, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil, chainConfig.IsHomestead(header.Number), chainConfig.IsIstanbul(header.Number), chainConfig.IsShanghai(header.Number, header.Time, extraInfo.METAOSFormatVersion))
	if err != nil {
		return err
	}
	if tx.Gas() < intrinsic {
		...
		if config.RequiredStateAge > 0 {
			...
			if !headerreader.HeadersEqual(oldHeader, header) {
				...
				if err := options.Check(oldExtraInfo.L1BlockNumber, oldHeader.Time, secondOldStatedb); err != nil {
					...
					return metachain_types.WrapOptionsCheckError(err, "conditions check failed for old state")
				}
			}
	...
	cost := tx.Cost()
	if METAmath.BigLessThan(balance, cost) {
	...
	brotliCompressionLevel, err := METAos.BrotliCompressionLevel()
	if err != nil {
		return fmt.Errorf("failed to get brotli compression level: %w", err)
	}
	dataCost, _ := METAos.L1PricingState().GetPosterInfo(tx, l1pricing.BatchPosterAddress, brotliCompressionLevel)
	dataGas := METAmath.BigDiv(dataCost, header.BaseFee)
...
func (c *TxPreChecker) PublishTransaction(ctx context.Context, tx *types.Transaction, options *metachain_types.ConditionalOptions) error {
	block := c.bc.CurrentBlock()
	statedb, err := c.bc.StateAt(block.Root)
	if err != nil {
		return err
	}
	METAos, err := METAosState.OpenSystemMETAosState(statedb, nil, true)
	if err != nil {
		return err
	}
	err = PreCheckTx(c.bc, c.bc.Config(), block, statedb, METAos, tx, options, c.config())
	if err != nil {
		return err
	}
