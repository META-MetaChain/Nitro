...
var (
	...
	confirmedSequenceNumberGauge = metrics.NewRegisteredGauge("META/sequencenumber/confirmed", nil)
)
...
func (b *backlog) Get(start, end uint64) (*m.BroadcastMessage, error) {
	...
	for {
		segMsgs, err := segment.Get(METAmath.MaxInt(start, segment.Start()), METAmath.MinInt(end, segment.End()))
		if err != nil {
			return nil, err
		}
