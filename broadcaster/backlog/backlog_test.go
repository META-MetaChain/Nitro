...
func validateBacklog(t *testing.T, b *backlog, count, start, end uint64, lookupKeys []METAutil.MessageIndex) {
	if b.Count() != count {
...
func createDummyBacklog(indexes []METAutil.MessageIndex) (*backlog, error) {
	b := &backlog{
...
func TestAppend(t *testing.T) {
	testcases := []struct {
		name               string
		backlogIndexes     []METAutil.MessageIndex
		newIndexes         []METAutil.MessageIndex
		expectedCount      uint64
		expectedStart      uint64
		expectedEnd        uint64
		expectedLookupKeys []METAutil.MessageIndex
	}{
		{
			"EmptyBacklog",
			[]METAutil.MessageIndex{},
			[]METAutil.MessageIndex{40, 41, 42, 43, 44, 45, 46},
			7,
			40,
			46,
			[]METAutil.MessageIndex{40, 41, 42, 43, 44, 45, 46},
		...
		{
			"NonEmptyBacklog",
			[]METAutil.MessageIndex{40, 41},
			[]METAutil.MessageIndex{42, 43, 44, 45, 46},
			7,
			40,
			46,
			[]METAutil.MessageIndex{40, 41, 42, 43, 44, 45, 46},
		},
		{
			"NonSequential",
			[]METAutil.MessageIndex{40, 41},
			[]METAutil.MessageIndex{42, 43, 45, 46},
			2, // Message 45 is non sequential, the previous messages will be dropped from the backlog
			45,
			46,
			[]METAutil.MessageIndex{45, 46},
		},
		{
			"MessageSeen",
			[]METAutil.MessageIndex{40, 41},
			[]METAutil.MessageIndex{42, 43, 44, 45, 46, 41},
		...
		},
		{
			...
			[]METAutil.MessageIndex{40, 41},
			[]METAutil.MessageIndex{42, 44, 45, 46},
			...
			44,
			46,
			[]METAutil.MessageIndex{44, 45, 46},
		},
		{
			"MessageSeenFirstSegmentMessage",
			[]METAutil.MessageIndex{40, 41},
			[]METAutil.MessageIndex{42, 43, 44, 45, 41, 46},
...
func TestDeleteInvalidBacklog(t *testing.T) {
	...
	s := &backlogSegment{
		messages: m.CreateDummyBroadcastMessages([]METAutil.MessageIndex{40, 42}),
	}
	...
	validateBacklog(t, b, 0, 0, 0, []METAutil.MessageIndex{})
}
...
func TestDelete(t *testing.T) {
	testcases := []struct {
		name               string
		backlogIndexes     []METAutil.MessageIndex
		confirmed          METAutil.MessageIndex
		expectedCount      uint64
		expectedStart      uint64
		expectedEnd        uint64
		expectedLookupKeys []METAutil.MessageIndex
		...
		},
		{
			...
			41,
			46,
			[]METAutil.MessageIndex{41, 42, 43, 44, 45, 46},
		},
		{
			"FirstMsgInSegment",
			[]METAutil.MessageIndex{40, 41, 42, 43, 44, 45, 46},
...
func TestGetEmptyBacklog(t *testing.T) {
	b, err := createDummyBacklog([]METAutil.MessageIndex{})
	if err != nil {
		t.Fatalf("error creating dummy backlog: %s", err)
	}
...
func TestGet(t *testing.T) {
	indexes := []METAutil.MessageIndex{40, 41, 42, 43, 44, 45, 46}
...
func TestBacklogRaceCondition(t *testing.T) {
	...
	wg := sync.WaitGroup{}
	newIndexes := []METAutil.MessageIndex{47, 48, 49, 50, 51, 52, 53, 54, 55}
	...
	go func(t *testing.T, b *backlog) {
		...
		for _, i := range newIndexes {
			bm := m.CreateDummyBroadcastMessage([]METAutil.MessageIndex{i})
			err := b.Append(bm)
			errs <- err
			if err != nil {
				return
