...
import (
	...
	"github.com/META-MetaChain/nitro/broadcaster"
	m "github.com/META-MetaChain/nitro/broadcaster/message"
	"github.com/META-MetaChain/nitro/execution"
	"github.com/META-MetaChain/nitro/staker"
	"github.com/META-MetaChain/nitro/util/METAmath"
	"github.com/META-MetaChain/nitro/util/sharedmetrics"
	"github.com/META-MetaChain/nitro/util/stopwaiter"
)
...
type TransactionStreamer struct {
	...
	execLastMsgCount METAutil.MessageIndex
	...
	broadcasterQueuedMessages            []METAostypes.MessageWithMetadata
...
func (s *TransactionStreamer) ReorgTo(count METAutil.MessageIndex) error {
	return s.ReorgToAndEndBatch(s.db.NewBatch(), count)
}

func (s *TransactionStreamer) ReorgToAndEndBatch(batch ethdb.Batch, count METAutil.MessageIndex) error {
...
func (s *TransactionStreamer) reorg(batch ethdb.Batch, count METAutil.MessageIndex, newMessages []METAostypes.MessageWithMetadata) error {
	if count == 0 {
	...
	var oldMessages []*METAostypes.MessageWithMetadata
	...
	config := s.config()
	maxResequenceMsgCount := count + METAutil.MessageIndex(config.MaxReorgResequenceDepth)
...
func setMessageCount(batch ethdb.KeyValueWriter, count METAutil.MessageIndex) error {
...
func (s *TransactionStreamer) GetMessage(seqNum METAutil.MessageIndex) (*METAostypes.MessageWithMetadata, error) {
	key := dbKey(messagePrefix, uint64(seqNum))
	data, err := s.db.Get(key)
	if err != nil {
		return nil, err
	}
	var message METAostypes.MessageWithMetadata
...
// Note: if changed to acquire the mutex, some internal users may need to be updated to a non-locking version.
func (s *TransactionStreamer) GetMessageCount() (METAutil.MessageIndex, error) {
	...
	return METAutil.MessageIndex(pos), nil
}

func (s *TransactionStreamer) GetProcessedMessageCount() (METAutil.MessageIndex, error) {
...
func (s *TransactionStreamer) AddMessages(pos METAutil.MessageIndex, messagesAreConfirmed bool, messages []METAostypes.MessageWithMetadata) error {
...
func (s *TransactionStreamer) AddBroadcastMessages(feedMessages []*m.BroadcastFeedMessage) error {
	...
	var messages []METAostypes.MessageWithMetadata
	...
	broadcastStartPos += METAutil.MessageIndex(dups)
	if oldMsg != nil {
	...
	if len(s.broadcasterQueuedMessages) == 0 || (feedReorg && !s.broadcasterQueuedMessagesActiveReorg) {
		...
		broadcasterQueuedMessagesPos := METAutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
		if broadcasterQueuedMessagesPos >= broadcastStartPos {
			// Feed messages older than cache
			s.broadcasterQueuedMessages = messages
			atomic.StoreUint64(&s.broadcasterQueuedMessagesPos, uint64(broadcastStartPos))
			s.broadcasterQueuedMessagesActiveReorg = feedReorg
		} else if broadcasterQueuedMessagesPos+METAutil.MessageIndex(len(s.broadcasterQueuedMessages)) == broadcastStartPos {
			...
			if len(s.broadcasterQueuedMessages) > 0 {
				log.Warn(
					...
					"expectedNextPos", broadcasterQueuedMessagesPos+METAutil.MessageIndex(len(s.broadcasterQueuedMessages)),
...
func (s *TransactionStreamer) AddFakeInitMessage() error {
	...
	chainIdBytes := METAmath.U256Bytes(s.chainConfig.ChainID)
	msg := append(append(chainIdBytes, 0), chainConfigJson...)
	return s.AddMessages(0, false, []METAostypes.MessageWithMetadata{{
		Message: &METAostypes.L1IncomingMessage{
			Header: &METAostypes.L1IncomingMessageHeader{
				Kind:      METAostypes.L1MessageType_Initialize,
...
// Used in redis tests
func (s *TransactionStreamer) GetMessageCountSync(t *testing.T) (METAutil.MessageIndex, error) {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
...
func (s *TransactionStreamer) AddMessagesAndEndBatch(pos METAutil.MessageIndex, messagesAreConfirmed bool, messages []METAostypes.MessageWithMetadata, batch ethdb.Batch) error {
...
func (s *TransactionStreamer) getPrevPrevDelayedRead(pos METAutil.MessageIndex) (uint64, error) {
...
func (s *TransactionStreamer) countDuplicateMessages(
	...
	messages []METAostypes.MessageWithMetadata,
	batch *ethdb.Batch,
) (int, bool, *METAostypes.MessageWithMetadata, error) {
	curMsg := 0
	for {
		...
		if !bytes.Equal(haveMessage, wantMessage) {
			...
			var dbMessageParsed METAostypes.MessageWithMetadata
			...
			if nextMessage.Message != nil {
				if dbMessageParsed.Message.BatchGasCost == nil || nextMessage.Message.BatchGasCost == nil {
					...
					nextMessageCopy.Message = new(METAostypes.L1IncomingMessage)
...
func (s *TransactionStreamer) logReorg(pos METAutil.MessageIndex, dbMsg *METAostypes.MessageWithMetadata, newMsg *METAostypes.MessageWithMetadata, confirmed bool) {
	sendLog := confirmed
...
func (s *TransactionStreamer) addMessagesAndEndBatchImpl(messageStartPos METAutil.MessageIndex, messagesAreConfirmed bool, messages []METAostypes.MessageWithMetadata, batch ethdb.Batch) error {
	var confirmedReorg bool
	var oldMsg *METAostypes.MessageWithMetadata
	var lastDelayedRead uint64
	var hasNewConfirmedMessages bool
	var cacheClearLen int

	messagesAfterPos := messageStartPos + METAutil.MessageIndex(len(messages))
	broadcastStartPos := METAutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
	...
	if messagesAreConfirmed {
		...
		if duplicates > 0 {
			...
			messageStartPos += METAutil.MessageIndex(duplicates)
		}
		if len(messages) > 0 {
	...
	for i, msg := range messages {
		msgPos := messageStartPos + METAutil.MessageIndex(i)
...
func (s *TransactionStreamer) WriteMessageFromSequencer(pos METAutil.MessageIndex, msgWithMeta METAostypes.MessageWithMetadata) error {
	...
	if err := s.writeMessages(pos, []METAostypes.MessageWithMetadata{msgWithMeta}, nil); err != nil {
		return err
	}
...
func (s *TransactionStreamer) GenesisBlockNumber() uint64 {
	return s.chainConfig.metachainChainParams.GenesisBlockNum
}
...
func (s *TransactionStreamer) writeMessage(pos METAutil.MessageIndex, msg METAostypes.MessageWithMetadata, batch ethdb.Batch) error {
	key := dbKey(messagePrefix, uint64(pos))
...
func (s *TransactionStreamer) writeMessages(pos METAutil.MessageIndex, messages []METAostypes.MessageWithMetadata, batch ethdb.Batch) error {
	if batch == nil {
		batch = s.db.NewBatch()
	}
	for i, msg := range messages {
		err := s.writeMessage(pos+METAutil.MessageIndex(i), msg, batch)
		if err != nil {
			return err
		}
	}

	err := setMessageCount(batch, pos+METAutil.MessageIndex(len(messages)))
	if err != nil {
		return err
	}
	err = batch.Write()
...
func (s *TransactionStreamer) ResultAtCount(count METAutil.MessageIndex) (*execution.MessageResult, error) {
	if count == 0 {
