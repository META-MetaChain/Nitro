...
func NewTransactionStreamerForTest(t *testing.T, ownerAddress common.Address) (*gethexec.ExecutionEngine, *TransactionStreamer, ethdb.Database, *core.BlockChain) {
	...
	bc, err := gethexec.WriteOrTestBlockChain(chainDb, nil, initReader, chainConfig, METAostypes.TestInitMessage, gethexec.ConfigDefaultTest().TxLookupLimit, 0)
	...
	inbox, err := NewTransactionStreamer(METADb, bc.Config(), execSeq, nil, make(chan error, 1), transactionStreamerConfigFetcher)
	if err != nil {
		Fail(t, err)
	}
	...
	return execEngine, inbox, METADb, bc
}

type blockTestState struct {
	balances    map[common.Address]*big.Int
	accounts    []common.Address
	numMessages METAutil.MessageIndex
	blockNumber uint64
}
	...
	for i := 1; i < 100; i++ {
		if i%10 == 0 {
			...
			for j := 0; j < numMessages; j++ {
				...
				var l2Message []byte
				l2Message = append(l2Message, METAos.L2MessageKind_ContractTx)
				l2Message = append(l2Message, METAmath.Uint64ToU256Bytes(gas)...)
				l2Message = append(l2Message, METAmath.Uint64ToU256Bytes(l2pricing.InitialBaseFeeWei)...)
				l2Message = append(l2Message, dest.Hash().Bytes()...)
				l2Message = append(l2Message, METAmath.U256Bytes(value)...)
				var requestId common.Hash
				binary.BigEndian.PutUint64(requestId.Bytes()[:8], uint64(i))
				messages = append(messages, METAostypes.MessageWithMetadata{
			...
			state.numMessages += METAutil.MessageIndex(len(messages))
			...
			for blockNum := prevBlockNumber + 1; blockNum <= state.blockNumber; blockNum++ {
				...
				for i, receipt := range receipts {
					...
					balance.Sub(balance, METAmath.BigMulByUint(block.BaseFee(), receipt.GasUsed))
				}
			}
