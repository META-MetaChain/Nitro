...
func NewInboxReader(tracker *InboxTracker, client METAutil.L1Interface, l1Reader *headerreader.HeaderReader, firstMessageBlock *big.Int, delayedBridge *DelayedBridge, sequencerInbox *SequencerInbox, config InboxReaderConfigFetcher) (*InboxReader, error) {
	err := config().Validate()
	if err != nil {
		return nil, err
	}
	...
	for i := 0; ; i++ {
		...
		if batchCount > 0 {
			...
			if initMessage.ChainConfig != nil {
				if err := initMessage.ChainConfig.CheckCompatible(chainConfig, chainConfig.metachainChainParams.GenesisBlockNum, 0); err != nil {
...
func (r *InboxReader) recentParentChainBlockToMsg(ctx context.Context, parentChainBlock uint64) (METAutil.MessageIndex, error) {
...
func (r *InboxReader) GetSafeMsgCount(ctx context.Context) (METAutil.MessageIndex, error) {
...
func (r *InboxReader) GetFinalizedMsgCount(ctx context.Context) (METAutil.MessageIndex, error) {
...
func (r *InboxReader) run(ctx context.Context, hadError bool) error {
	...
	for {
		...
		neededBlockAdvance := config.DelayBlocks + METAmath.SaturatingUSub(config.MinBlocksToRead, 1)
		neededBlockHeight := METAmath.BigAddByUint(from, neededBlockAdvance)
		checkDelayTimer := time.NewTimer(config.CheckDelay)
	WaitForHeight:
		for METAmath.BigLessThan(currentHeight, neededBlockHeight) {
			select {
		...
		if !missingDelayed && !reorgingDelayed && !missingSequencer && !reorgingSequencer {
			...
			from = METAmath.BigAddByUint(currentHeight, 1)
		...
		for {
			...
			if reorgingDelayed || reorgingSequencer {
				...
				from = METAmath.BigAddByUint(to, 1)
			}
...
func (r *InboxReader) getPrevBlockForReorg(from *big.Int) (*big.Int, error) {
	...
	newFrom := METAmath.BigSub(from, big.NewInt(10))
...
func (r *InboxReader) getNextBlockToRead() (*big.Int, error) {
	...
	if METAmath.BigLessThan(msgBlock, r.firstMessageBlock) {
...
func (r *InboxReader) GetSequencerMessageBytes(ctx context.Context, seqNum uint64) ([]byte, error) {
	...
	blockNum := METAmath.UintToBig(metadata.ParentChainBlock)
