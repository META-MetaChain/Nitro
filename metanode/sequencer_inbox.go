...
func NewSequencerInbox(client METAutil.L1Interface, addr common.Address, fromBlock int64) (*SequencerInbox, error) {
...
func (m *SequencerInboxBatch) getSequencerData(ctx context.Context, client METAutil.L1Interface) ([]byte, error) {
	switch m.dataLocation {
	case batchDataTxInput:
		data, err := METAutil.GetLogEmitterTxData(ctx, client, m.rawLog)
		if err != nil {
			return nil, err
		}
...
func (m *SequencerInboxBatch) Serialize(ctx context.Context, client METAutil.L1Interface) ([]byte, error) {
	if m.serialized != nil {
