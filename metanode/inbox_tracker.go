...
var (
	inboxLatestBatchGauge        = metrics.NewRegisteredGauge("META/inbox/latest/batch", nil)
	inboxLatestBatchMessageGauge = metrics.NewRegisteredGauge("META/inbox/latest/batch/message", nil)
)
...
type InboxTracker struct {
	...
	das        METAstate.DataAvailabilityReader

	batchMetaMutex sync.Mutex
	batchMeta      *containers.LruCache[uint64, BatchMetadata]
}

func NewInboxTracker(db ethdb.Database, txStreamer *TransactionStreamer, das METAstate.DataAvailabilityReader) (*InboxTracker, error) {
	// We support a nil txStreamer for the pruning code
	if txStreamer != nil && txStreamer.chainConfig.metachainChainParams.DataAvailabilityCommittee && das == nil {
...
func (t *InboxTracker) GetBatchMessageCount(seqNum uint64) (METAutil.MessageIndex, error) {
...
func (t *InboxTracker) PopulateFeedBacklog(broadcastServer *broadcaster.Broadcaster) error {
	...
	var startMessage METAutil.MessageIndex
	if batchCount >= 2 {
...
func (t *InboxTracker) legacyGetDelayedMessageAndAccumulator(seqNum uint64) (*METAostypes.L1IncomingMessage, common.Hash, error) {
	key := dbKey(legacyDelayedMessagePrefix, seqNum)
	data, err := t.db.Get(key)
	if err != nil {
	...
	var acc common.Hash
	copy(acc[:], data[:32])
	msg, err := METAostypes.ParseIncomingL1Message(bytes.NewReader(data[32:]), nil)
	return msg, acc, err
}

func (t *InboxTracker) GetDelayedMessageAccumulatorAndParentChainBlockNumber(seqNum uint64) (*METAostypes.L1IncomingMessage, common.Hash, uint64, error) {
	...
	var acc common.Hash
	copy(acc[:], data[:32])
	var msg *METAostypes.L1IncomingMessage
...
func (t *InboxTracker) GetDelayedMessage(seqNum uint64) (*METAostypes.L1IncomingMessage, error) {
...
func (t *InboxTracker) setDelayedCountReorgAndWriteBatch(batch ethdb.Batch, newDelayedCount uint64, canReorgBatches bool) error {
	...
	var prevMesssageCount METAutil.MessageIndex
	if count > 0 {
...
func (b *multiplexerBackend) ReadDelayedInbox(seqNum uint64) (*METAostypes.L1IncomingMessage, error) {
...
func (t *InboxTracker) AddSequencerBatches(ctx context.Context, client METAutil.L1Interface, batches []*SequencerInboxBatch) error {
	if len(batches) == 0 {
		return nil
	}
	t.mutex.Lock()
	...
	multiplexer := METAstate.NewInboxMultiplexer(backend, prevbatchmeta.DelayedMessageCount, t.das, METAstate.KeysetValidate)
	batchMessageCounts := make(map[uint64]METAutil.MessageIndex)
	...
	newMessageCount := prevbatchmeta.MessageCount + METAutil.MessageIndex(len(messages))
