...
import (
	...
	"github.com/META-MetaChain/nitro/broadcastclient"
	"github.com/META-MetaChain/nitro/broadcastclients"
...
func checkMETADbSchemaVersion(METADb ethdb.Database) error {
	var version uint64
	hasVersion, err := METADb.Has(dbSchemaVersion)
	if err != nil {
		return err
	}
	if hasVersion {
		versionBytes, err := METADb.Get(dbSchemaVersion)
		if err != nil {
			return err
		}
...
func createNodeImpl(
	...
	err := checkMETADbSchemaVersion(METADb)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		if l2Config.metachainChainParams.GenesisBlockNum > 0 {
	...
	if config.ParentChainReader.Enable {
		...
		l1Reader, err = headerreader.New(ctx, l1client, func() *headerreader.Config { return &configFetcher.Get().ParentChainReader }, METASys)
		if err != nil {
			return nil, err
		}
	}
	...
	txStreamer, err := NewTransactionStreamer(METADb, l2Config, exec, broadcastServer, fatalErrChan, transactionStreamerConfigFetcher)
	if err != nil {
		return nil, err
	}
	...
	} else if l2Config.metachainChainParams.DataAvailabilityCommittee {
		return nil, errors.New("a data availability service is required for this chain, but it was not configured")
	}

	inboxTracker, err := NewInboxTracker(METADb, txStreamer, daReader)
	if err != nil {
		return nil, err
	}
	...
	if config.BlockValidator.ValidationServer.URL != "" {
		statelessBlockValidator, err = staker.NewStatelessBlockValidator(
			...
			txStreamer,
			exec,
			rawdb.NewTable(METADb, storage.BlockValidatorPrefix),
			daReader,
	...
	if config.Staker.Enable {
		dp, err := StakerDataposter(
			ctx,
			rawdb.NewTable(METADb, storage.StakerPrefix),
			l1Reader,
			txOptsValidator,
			configFetcher,
			syncMonitor,
	...
	if config.BatchPoster.Enable {
		...
		batchPoster, err = NewBatchPoster(ctx, &BatchPosterOpts{
			DataPosterDB: rawdb.NewTable(METADb, storage.BatchPosterPrefix),
			L1Reader:     l1Reader,
			Inbox:        inboxTracker,
			Streamer:     txStreamer,
			SyncMonitor:  syncMonitor,
...
func CreateNode(
	...
	currentNode, err := createNodeImpl(ctx, stack, exec, METADb, configFetcher, l2Config, l1client, deployInfo, txOptsValidator, txOptsBatchPoster, dataSigner, fatalErrChan)
	if err != nil {
		return nil, err
	}
	var apis []rpc.API
