...
func (s *SyncMonitor) SyncProgressMap() map[string]interface{} {
	...
	if err != nil {
		...
		if builtMessageCount+METAutil.MessageIndex(s.config.BlockBuildLag) < msgCount {
			syncing = true
		}
	}
	...
	if s.inboxReader != nil {
		...
		if err != nil {
			...
			if builtMessageCount+METAutil.MessageIndex(s.config.BlockBuildSequencerInboxLag) < processedMetadata.MessageCount {
				syncing = true
			}
		}
	...
	if s.coordinator != nil {
		...
		if err != nil {
			...
			if msgCount+METAutil.MessageIndex(s.config.CoordinatorMsgLag) < coordinatorMessageCount {
				syncing = true
			}
		}
	}
