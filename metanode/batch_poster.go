...
import (
	...
	"github.com/META-MetaChain/nitro/METAnode/dataposter"
	"github.com/META-MetaChain/nitro/METAnode/redislock"
...
var (
	batchPosterWalletBalance      = metrics.NewRegisteredGaugeFloat64("META/batchposter/wallet/balanceether", nil)
	batchPosterGasRefunderBalance = metrics.NewRegisteredGaugeFloat64("META/batchposter/gasrefunder/balanceether", nil)
	batchPosterSimpleRedisLockKey = "node.batch-poster.redis-lock.simple-lock-key"
)

type batchPosterPosition struct {
	MessageCount        METAutil.MessageIndex
...
type BatchPoster struct {
	...
	messagesPerBatch    *METAmath.MovingAverage[uint64]
...
func NewBatchPoster(ctx context.Context, opts *BatchPosterOpts) (*BatchPoster, error) {
	...
	b.messagesPerBatch, err = METAmath.NewMovingAverage[uint64](20)
	if err != nil {
		return nil, err
	}
	...
	b.accessList = func(SequencerInboxAccs, AfterDelayedMessagesRead int) types.AccessList {
		if opts.L1Reader.IsParentChainmetachain() {
...
func AccessList(opts *AccessListOpts) types.AccessList {
	l := types.AccessList{
		types.AccessTuple{
			...
			StorageKeys: []common.Hash{
				...
				common.Hash(METAutil.PaddedKeccak256(opts.DataPosterAddr.Bytes(), []byte{3})),
			},
		},
		types.AccessTuple{
	...
	for _, v := range []struct{ slotIdx, val int }{
		...
		sb := METAutil.SumBytes(METAutil.PaddedKeccak256([]byte{byte(v.slotIdx)}), big.NewInt(int64(v.val)).Bytes())
	...
	if (opts.GasRefunderAddr != common.Address{}) {
		l = append(l, types.AccessTuple{
			...
			StorageKeys: []common.Hash{
				...
				common.Hash(METAutil.PaddedKeccak256(opts.SequencerInboxAddr.Bytes(), []byte{1})),
				// allowedRefundees[refundee]; for mainnet it's: "0xe85fd79f89ff278fc57d40aecb7947873df9f0beac531c8f71a98f630e1eab62".
				common.Hash(METAutil.PaddedKeccak256(opts.DataPosterAddr.Bytes(), []byte{2})),
			},
		})
	}
	return l
...
type buildingBatch struct {
	...
	startMsgCount     METAutil.MessageIndex
	msgCount          METAutil.MessageIndex
...
func newBatchSegments(firstDelayed uint64, config *BatchPosterConfig, backlog uint64) *batchSegments {
	...
	if backlog > 20 {
		compressionLevel = METAmath.MinInt(compressionLevel, brotli.DefaultCompression)
	}
	if backlog > 40 {
		recompressionLevel = METAmath.MinInt(recompressionLevel, brotli.DefaultCompression)
	}
	if backlog > 60 {
		compressionLevel = METAmath.MinInt(compressionLevel, 4)
	}
...
func (s *batchSegments) recompressAll() error {
	...
	if s.totalUncompressedSize > METAstate.MaxDecompressedLen {
		return fmt.Errorf("batch size %v exceeds maximum decompressed length %v", s.totalUncompressedSize, METAstate.MaxDecompressedLen)
	}
	if len(s.rawSegments) >= METAstate.MaxSegmentsPerSequencerMessage {
		return fmt.Errorf("number of raw segments %v excees maximum number %v", len(s.rawSegments), METAstate.MaxSegmentsPerSequencerMessage)
	}
	return nil
}
...
func (s *batchSegments) addL2Msg(l2msg []byte) (bool, error) {
	...
	segment[0] = METAstate.BatchSegmentKindL2Message
...
func (s *batchSegments) addDelayedMessage() (bool, error) {
	segment := []byte{METAstate.BatchSegmentKindDelayedMessages}
...
func (s *batchSegments) AddMessage(msg *METAostypes.MessageWithMetadata) (bool, error) {
	if s.isDone {
		return false, errBatchAlreadyClosed
	}
	...
	success, err := s.maybeAddDiffSegment(&s.timestamp, msg.Message.Header.Timestamp, METAstate.BatchSegmentKindAdvanceTimestamp)
	if !success {
		return false, err
	}
	success, err = s.maybeAddDiffSegment(&s.blockNum, msg.Message.Header.BlockNumber, METAstate.BatchSegmentKindAdvanceL1BlockNumber)
	if !success {
		return false, err
	}
...
func (s *batchSegments) CloseAndGetBytes() ([]byte, error) {
	...
	fullMsg[0] = METAstate.BrotliMessageHeaderByte
	fullMsg = append(fullMsg, compressedBytes...)
	return fullMsg, nil
}

func (b *BatchPoster) encodeAddBatch(seqNum *big.Int, prevMsgNum METAutil.MessageIndex, newMsgNum METAutil.MessageIndex, message []byte, delayedMsg uint64) ([]byte, error) {
...
func (b *BatchPoster) maybePostSequencerBatch(ctx context.Context) (bool, error) {
	...
	if hasL1Bound {
		...
		l1BoundBlockNumber := METAutil.ParentHeaderToL1BlockNumber(l1Bound)
		l1BoundMaxBlockNumber = METAmath.SaturatingUAdd(l1BoundBlockNumber, METAmath.BigToUintSaturating(maxTimeVariation.FutureBlocks))
		l1BoundMaxTimestamp = METAmath.SaturatingUAdd(l1Bound.Time, METAmath.BigToUintSaturating(maxTimeVariation.FutureSeconds))
		...
		if config.L1BlockBoundBypass > 0 {
			...
			latestBlockNumber := METAutil.ParentHeaderToL1BlockNumber(latestHeader)
			blockNumberWithPadding := METAmath.SaturatingUAdd(latestBlockNumber, uint64(config.L1BlockBoundBypass/ethPosBlockTime))
			timestampWithPadding := METAmath.SaturatingUAdd(latestHeader.Time, uint64(config.L1BlockBoundBypass/time.Second))

			l1BoundMinBlockNumber = METAmath.SaturatingUSub(blockNumberWithPadding, METAmath.BigToUintSaturating(maxTimeVariation.DelayBlocks))
			l1BoundMinTimestamp = METAmath.SaturatingUSub(timestampWithPadding, METAmath.BigToUintSaturating(maxTimeVariation.DelaySeconds))
		}
	}
	...
	for b.building.msgCount < msgCount {
		...
		if msg.Message.Header.Kind != METAostypes.L1MessageType_BatchPostingReport {
			b.building.haveUsefulMessage = true
		}
		b.building.msgCount++
	}
	...
	b.CallIteratively(func(ctx context.Context) time.Duration {
		...
		if common.HexToAddress(b.config().GasRefunderAddress) != (common.Address{}) {
			...
			if err != nil {
				...
				batchPosterGasRefunderBalance.Update(METAmath.BalancePerEther(gasRefunderBalance))
			}
		}
		if b.dataPoster.Sender() != (common.Address{}) {
			...
			if err != nil {
				...
				batchPosterWalletBalance.Update(METAmath.BalancePerEther(walletBalance))
			}
		}
