...
var (
	isActiveSequencer = metrics.NewRegisteredGauge("META/sequencer/active", nil)
)
...
type SeqCoordinatorConfig struct {
	...
	MsgPerPoll METAutil.MessageIndex       `koanf:"msg-per-poll"`
...
func StandaloneSeqCoordinatorInvalidateMsgIndex(ctx context.Context, redisClient redis.UniversalClient, keyConfig string, msgIndex METAutil.MessageIndex) error {
...
func (c *SeqCoordinator) msgCountToSignedBytes(msgCount METAutil.MessageIndex) ([]byte, error) {
...
func (c *SeqCoordinator) signedBytesToMsgCount(ctx context.Context, data []byte) (METAutil.MessageIndex, error) {
	datalen := len(data)
	if datalen < 8 {
	...
	return METAutil.MessageIndex(binary.BigEndian.Uint64(msgCountBytes)), nil
}

// Acquires or refreshes the chosen one lockout and optionally writes a message into redis atomically.
func (c *SeqCoordinator) acquireLockoutAndWriteMessage(ctx context.Context, msgCountExpected, msgCountToWrite METAutil.MessageIndex, lastmsg *METAostypes.MessageWithMetadata) error {
	...
	if lastmsg != nil {
		...
		msgSig, err := c.signer.SignMessage(METAmath.UintToBytes(uint64(msgCountToWrite-1)), msgBytes)
		if err != nil {
			return err
		}
...
func (c *SeqCoordinator) getRemoteMsgCountImpl(ctx context.Context, r redis.Cmdable) (METAutil.MessageIndex, error) {
...
func (c *SeqCoordinator) GetRemoteMsgCount() (METAutil.MessageIndex, error) {
...
func (c *SeqCoordinator) updateWithLockout(ctx context.Context, nextChosen string) time.Duration {
	...
	marginOfError := METAmath.MaxInt(c.config.LockoutDuration/5, c.config.UpdateInterval*5)
...
func (c *SeqCoordinator) update(ctx context.Context) time.Duration {
	...
	for msgToRead < readUntil {
		...
		msgReadErr = c.signer.VerifySignature(ctx, sigBytes, METAmath.UintToBytes(uint64(msgToRead)), rsBytes)
		if msgReadErr != nil {
		...
		if err != nil {
			...
			message = METAostypes.MessageWithMetadata{
				Message:             METAostypes.InvalidL1Message,
...
func (c *SeqCoordinator) SequencingMessage(pos METAutil.MessageIndex, msg *METAostypes.MessageWithMetadata) error {
