...
import (
	...
	"github.com/META-MetaChain/nitro/METAnode/dataposter/dbstorage"
	"github.com/META-MetaChain/nitro/METAnode/dataposter/noop"
	"github.com/META-MetaChain/nitro/METAnode/dataposter/slice"
	...
	redisstorage "github.com/META-MetaChain/nitro/METAnode/dataposter/redis"
)
...
func NewDataPoster(ctx context.Context, opts *DataPosterOpts) (*DataPoster, error) {
	...
	if opts.HeaderReader.IsParentChainmetachain() && !cfg.UseNoOpStorage {
		useNoOpStorage = true
		log.Info("Disabling data poster storage, as parent chain appears to be an metachain chain without a mempool")
	}
...
func (p *DataPoster) waitForL1Finality() bool {
	return p.config().WaitForL1Finality && !p.headerReader.IsParentChainmetachain()
}
...
func (p *DataPoster) getNextNonceAndMaybeMeta(ctx context.Context) (uint64, []byte, bool, error) {
	...
	if err := p.updateNonce(ctx); err != nil {
		...
		nonceQueryBlock := METAmath.UintToBig(METAmath.SaturatingUSub(blockNum, 1))
...
const minRbfIncrease = METAmath.OneInBips * 11 / 10
...
func (p *DataPoster) evalMaxFeeCapExpr(backlogOfBatches uint64, elapsed time.Duration) (*big.Int, error) {
	...
	resultBig := METAmath.FloatToBig(resultFloat * params.GWei)
	if resultBig == nil {
...
func (p *DataPoster) feeAndTipCaps(ctx context.Context, nonce uint64, gasLimit uint64, lastFeeCap *big.Int, lastTipCap *big.Int, dataCreatedAt time.Time, dataPosterBacklog uint64) (*big.Int, *big.Int, error) {
	...
	softConfBlock := METAmath.BigSubByUint(latestHeader.Number, config.NonceRbfSoftConfs)
	...
	newFeeCap = METAmath.BigMax(newFeeCap, METAmath.FloatToBig(config.MinFeeCapGwei*params.GWei))

	newTipCap, err := p.client.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, nil, err
	}
	newTipCap = METAmath.BigMax(newTipCap, METAmath.FloatToBig(config.MinTipCapGwei*params.GWei))
	newTipCap = METAmath.BigMin(newTipCap, METAmath.FloatToBig(config.MaxTipCapGwei*params.GWei))

	hugeTipIncrease := false
	if lastTipCap != nil {
		newTipCap = METAmath.BigMax(newTipCap, METAmath.BigMulByBips(lastTipCap, minRbfIncrease))
		// hugeTipIncrease is true if the new tip cap is at least 10x the last tip cap
		hugeTipIncrease = lastTipCap.Sign() == 0 || METAmath.BigDiv(newTipCap, lastTipCap).Cmp(big.NewInt(10)) >= 0
	}
	...
	if lastFeeCap != nil && hugeTipIncrease {
		...
		newFeeCap = METAmath.BigMax(newFeeCap, METAmath.BigMulByBips(lastFeeCap, minRbfIncrease))
	}
	...
	if METAmath.BigGreaterThan(newFeeCap, maxFeeCap) {
		log.Warn(
	...
	if config.AllocateMempoolBalance && !p.usingNoOpStorage {
		...
		if nonce != softConfNonce && config.MaxMempoolTransactions > 1 {
			...
			balanceForTx.Div(balanceForTx, METAmath.UintToBig(config.MaxMempoolTransactions-1))
		}
	}
	balanceFeeCap := METAmath.BigDivByUint(balanceForTx, gasLimit)
	if METAmath.BigGreaterThan(newFeeCap, balanceFeeCap) {
		log.Warn(
	...
	if METAmath.BigGreaterThan(newTipCap, newFeeCap) {
		log.Warn(
...
func (p *DataPoster) replaceTx(ctx context.Context, prevTx *storage.QueuedTransaction, backlogOfBatches uint64) error {
	...
	minNewFeeCap := METAmath.BigMulByBips(prevTx.Data.GasFeeCap, minRbfIncrease)
	newTx := *prevTx
...
func (p *DataPoster) updateNonce(ctx context.Context) error {
	...
	if p.lastBlock != nil && METAmath.BigEquals(p.lastBlock, header.Number) {
		return nil
	}
...
type DataPosterConfig struct {
	...
	// This is forcibly disabled if the parent chain is an metachain chain,
