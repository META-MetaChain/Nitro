...
func TestMessagePrunerWithPruningEligibleMessagePresent(t *testing.T) {
	...
	err := pruner.deleteOldMessagesFromDB(ctx, METAutil.MessageIndex(messagesCount), messagesCount)
	Require(t, err)
...
func TestMessagePrunerTwoHalves(t *testing.T) {
	...
	err := pruner.deleteOldMessagesFromDB(ctx, METAutil.MessageIndex(messagesCount/2), messagesCount/2)
	Require(t, err)
