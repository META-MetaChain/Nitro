...
func NewDelayedBridge(client METAutil.L1Interface, addr common.Address, fromBlock uint64) (*DelayedBridge, error) {
...
func (m *DelayedInboxMessage) AfterInboxAcc() common.Hash {
	hash := crypto.Keccak256(
		...
		METAmath.UintToBytes(m.Message.Header.BlockNumber),
		METAmath.UintToBytes(m.Message.Header.Timestamp),
		m.Message.Header.RequestId.Bytes(),
		METAmath.U256Bytes(m.Message.Header.L1BaseFee),
		crypto.Keccak256(m.Message.L2msg),
	)
	return crypto.Keccak256Hash(m.BeforeInboxAcc[:], hash)
}

func (b *DelayedBridge) LookupMessagesInRange(ctx context.Context, from, to *big.Int, batchFetcher METAostypes.FallibleBatchFetcher) ([]*DelayedInboxMessage, error) {
...
func (b *DelayedBridge) logsToDeliveredMessages(ctx context.Context, logs []types.Log, batchFetcher METAostypes.FallibleBatchFetcher) ([]*DelayedInboxMessage, error) {
	if len(logs) == 0 {
		return nil, nil
	}
	...
	for _, parsedLog := range parsedLogs {
		...
		if lastParentChainBlockNumber == parentChainBlockNumber && lastParentChainBlockNumber > 0 {
			...
			var err error
			l1BlockNumber, err = METAutil.CorrespondingL1BlockNumber(ctx, b.client, parentChainBlockNumber)
			if err != nil {
				return nil, err
			}
...
func (b *DelayedBridge) parseMessage(ctx context.Context, ethLog types.Log) (*big.Int, []byte, error) {
	...
	case ethLog.Topics[0] == inboxMessageFromOriginID:
		...
		data, err := METAutil.GetLogEmitterTxData(ctx, b.client, ethLog)
		if err != nil {
			return nil, nil, err
		}
