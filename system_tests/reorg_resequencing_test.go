...
func TestReorgResequencing(t *testing.T) {
	...
	newMessage := &METAostypes.L1IncomingMessage{
		...
		L2msg: append(builder.L2Info.GetAddress("User4").Bytes(), METAmath.Uint64ToU256Bytes(params.Ether)...),
	}
	err = builder.L2.ConsensusNode.TxStreamer.AddMessages(startMsgCount, true, []METAostypes.MessageWithMetadata{{
		Message:             newMessage,
