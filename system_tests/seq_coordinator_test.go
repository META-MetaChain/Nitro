...
func TestRedisSeqCoordinatorPriorities(t *testing.T) {
	...
	trySequencing := func(nodeNum int) bool {
		...
		Require(t, err)
		emptyMessage := METAostypes.MessageWithMetadata{
		...
		Require(t, err)
		Require(t, node.TxStreamer.AddMessages(curMsgs, false, []METAostypes.MessageWithMetadata{emptyMessage}))
		return true
	}
	...
	waitForMsgEverywhere := func(msgNum METAutil.MessageIndex) {
	...
	messagesPerRound := METAutil.MessageIndex(10)
	currentSequencer := 0
	sequencedMesssages := METAutil.MessageIndex(1) // we start with 1 so messageCountKey will be written
	...
	for {
		...
		for i := METAutil.MessageIndex(0); i < messagesPerRound; i++ {
