...
func TestEstimate(t *testing.T) {
	...
	Require(t, err, "could not deploy METAOwner contract")
	tx, err := METAOwner.SetMinimumL2BaseFee(&auth, gasPrice)
	Require(t, err, "could not set L2 gas price")
	_, err = builder.L2.EnsureTxSucceeded(tx)
	Require(t, err)

	// connect to METAGasInfo precompile
	...
	for !equilibrated && numTriesLeft > 0 {
		...
		_, _, _, _, _, setPrice, err := METAGasInfo.GetPricesInWei(&bind.CallOpts{})
...
func TestDifficultyForLatestMETAOS(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestDifficultyForMETAOSTen(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	builder := NewNodeBuilder(ctx).DefaultConfig(t, false)
	builder.chainConfig.metachainChainParams.InitialMETAOSVersion = 10
	cleanup := builder.Build(t)
	defer cleanup()
...
func TestComponentEstimate(t *testing.T) {
	...
	l1BaseFee := new(big.Int).Set(METAostypes.DefaultInitialL1BaseFee)
	...
	maxFeePerGas := METAmath.BigMulByUfrac(l2BaseFee, 3, 2)
	...
	tx := builder.L2Info.SignTxAs("User", &types.DynamicFeeTx{
		ChainID:   builder.L2.ExecNode.METAInterface.BlockChain().Config().ChainID,
		Nonce:     0,
		GasTipCap: maxPriorityFeePerGas,
		GasFeeCap: maxFeePerGas,
		Gas:       gasEstimate,
	...
	if !METAmath.BigEquals(l1BaseFeeEstimate, l1BaseFee) {
		Fatal(t, l1BaseFeeEstimate, l1BaseFee)
	}
	if !METAmath.BigEquals(baseFee, l2BaseFee) {
