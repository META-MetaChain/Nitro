...
import (
	...
	"github.com/META-MetaChain/nitro/solgen/go/challengegen"
	"github.com/META-MetaChain/nitro/solgen/go/mocksgen"
	"github.com/META-MetaChain/nitro/solgen/go/ospgen"
...
func writeTxToBatch(writer io.Writer, tx *types.Transaction) error {
	...
	var segment []byte
	segment = append(segment, METAstate.BatchSegmentKindL2Message)
	segment = append(segment, METAos.L2MessageKind_SignedTx)
...
func makeBatch(t *testing.T, l2Node *METAnode.Node, l2Info *BlockchainTestInfo, backend *ethclient.Client, sequencer *bind.TransactOpts, seqInbox *mocksgen.SequencerInboxStub, seqInboxAddr common.Address, modStep int64) {
	...
	compressed, err := METAcompress.CompressWell(batchBuffer.Bytes())
	Require(t, err)
	...
	nodeSeqInbox, err := METAnode.NewSequencerInbox(backend, seqInboxAddr, 0)
	Require(t, err)
...
func confirmLatestBlock(ctx context.Context, t *testing.T, l1Info *BlockchainTestInfo, backend METAutil.L1Interface) {
...
func setupSequencerInboxStub(ctx context.Context, t *testing.T, l1Info *BlockchainTestInfo, l1Client METAutil.L1Interface, chainConfig *params.ChainConfig) (common.Address, *mocksgen.SequencerInboxStub, common.Address) {
...
func RunChallengeTest(t *testing.T, asserterIsCorrect bool, useStubs bool, challengeMsgIdx int64) {
	...
	conf := METAnode.ConfigDefaultL1Test()
	...
	if useStubs {
		mockSpawn, valStack = createMockValidationNode(t, ctx, &valnode.TestValidationConfig.METAitrator)
	} else {
	...
	asserterL2Info, asserterL2Stack, asserterL2ChainDb, asserterL2METADb, asserterL2Blockchain := createL2BlockChainWithStackConfig(t, nil, "", chainConfig, initMessage, nil, nil)
	...
	Require(t, err)
	asserterL2, err := METAnode.CreateNode(ctx, asserterL2Stack, asserterExec, asserterL2METADb, NewFetcherFromConfig(conf), chainConfig, l1Backend, asserterRollupAddresses, nil, nil, nil, fatalErrChan)
	Require(t, err)
	err = asserterL2.Start(ctx)
	Require(t, err)

	challengerL2Info, challengerL2Stack, challengerL2ChainDb, challengerL2METADb, challengerL2Blockchain := createL2BlockChainWithStackConfig(t, nil, "", chainConfig, initMessage, nil, nil)
	...
	Require(t, err)
	challengerL2, err := METAnode.CreateNode(ctx, challengerL2Stack, challengerExec, challengerL2METADb, NewFetcherFromConfig(conf), chainConfig, l1Backend, &challengerRollupAddresses, nil, nil, nil, fatalErrChan)
	Require(t, err)
	...
	asserterGenesis := asserterExec.METAInterface.BlockChain().Genesis()
	challengerGenesis := challengerExec.METAInterface.BlockChain().Genesis()
	if asserterGenesis.Hash() != challengerGenesis.Hash() {
		Fatal(t, "asserter and challenger have different genesis hashes")
	}
	asserterLatestBlock := asserterExec.METAInterface.BlockChain().CurrentBlock()
	challengerLatestBlock := challengerExec.METAInterface.BlockChain().CurrentBlock()
	...
	asserterValidator, err := staker.NewStatelessBlockValidator(asserterL2.InboxReader, asserterL2.InboxTracker, asserterL2.TxStreamer, asserterExec.Recorder, asserterL2METADb, nil, StaticFetcherFrom(t, &conf.BlockValidator), valStack)
	if err != nil {
		Fatal(t, err)
	}
	if useStubs {
	...
	challengerValidator, err := staker.NewStatelessBlockValidator(challengerL2.InboxReader, challengerL2.InboxTracker, challengerL2.TxStreamer, challengerExec.Recorder, challengerL2METADb, nil, StaticFetcherFrom(t, &conf.BlockValidator), valStack)
	if err != nil {
		Fatal(t, err)
	}
	if useStubs {
