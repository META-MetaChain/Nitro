...
type BlockchainTestInfo struct {
	...
	METAInitData statetransfer.METAosInitializationInfo
	GasPrice    *big.Int
...
func NewMETATestInfo(t *testing.T, chainId *big.Int) *BlockchainTestInfo {
	var transferGas = util.NormalizeL2GasForL1GasInitial(800_000, params.GWei) // include room for aggregator L1 costs
	METAinfo := NewBlockChainTestInfo(
		t,
		types.NewmetachainSigner(types.NewLondonSigner(chainId)), big.NewInt(l2pricing.InitialBaseFeeWei*2),
		transferGas,
	)
	METAinfo.GenerateGenesisAccount("Owner", new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(9)))
	METAinfo.GenerateGenesisAccount("Faucet", new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(9)))
	return METAinfo
}
...
func (b *BlockchainTestInfo) GenerateGenesisAccount(name string, balance *big.Int) {
	...
	b.METAInitData.Accounts = append(b.METAInitData.Accounts, statetransfer.AccountInitializationInfo{
...
func (b *BlockchainTestInfo) GetGenesisAlloc() core.GenesisAlloc {
	...
	for _, info := range b.METAInitData.Accounts {
