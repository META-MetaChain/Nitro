...
import (
	...
	"github.com/META-MetaChain/nitro/util"
)

func prepareNodeWithHistory(t *testing.T, ctx context.Context, execConfig *gethexec.Config, txCount uint64) (node *METAnode.Node, executionNode *gethexec.ExecutionNode, l2client *ethclient.Client, cancel func()) {
	t.Helper()
...
func TestRecreateStateForRPCNoDepthLimit(t *testing.T) {
	...
	nodeConfig.RPC.MaxRecreateStateDepth = metachain.InfiniteMaxRecreateStateDepth
	...
	defer cancelNode()
	bc := execNode.Backend.METAInterface().BlockChain()
...
func TestRecreateStateForRPCDepthLimitExceeded(t *testing.T) {
	...
	if err.Error() != metachain.ErrDepthLimitExceeded.Error() {
...
func testSkippingSavingStateAndRecreatingAfterRestart(t *testing.T, cacheConfig *gethexec.CachingConfig, txCount int) {
	...
	l2info, stack, chainDb, METADb, blockchain := createL2BlockChain(t, nil, t.TempDir(), params.metachainDevTestChainConfig(), &execConfig.Caching)
	...
	node, err := METAnode.CreateNode(ctx1, stack, execNode, METADb, NewFetcherFromConfig(METAnode.ConfigDefaultL2Test()), blockchain.Config(), nil, nil, nil, nil, nil, feedErrChan)
	Require(t, err)
	...
	l2info, stack, chainDb, METADb, blockchain = createL2BlockChain(t, l2info, dataDir, params.metachainDevTestChainConfig(), &execConfig.Caching)

	execNode, err = gethexec.CreateExecutionNode(ctx1, stack, chainDb, blockchain, nil, execConfigFetcher)
	Require(t, err)

	node, err = METAnode.CreateNode(ctx, stack, execNode, METADb, NewFetcherFromConfig(METAnode.ConfigDefaultL2Test()), blockchain.Config(), nil, node.DeployInfo, nil, nil, nil, feedErrChan)
	Require(t, err)
