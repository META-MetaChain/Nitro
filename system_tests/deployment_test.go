...
func testContractDeployment(t *testing.T, ctx context.Context, client *ethclient.Client, contractCode []byte, accountInfo *AccountInfo, expectedEstimateGasError error) {
	...
	// len(contractCode)
	deployCode = append(deployCode, METAmath.Uint64ToU256Bytes(uint64(len(contractCode)))...)
...
func TestExtendedContractDeployment(t *testing.T) {
	...
	builder := NewNodeBuilder(ctx).DefaultConfig(t, false)
	builder.chainConfig.metachainChainParams.MaxCodeSize = params.DefaultMaxCodeSize * 3
	builder.chainConfig.metachainChainParams.MaxInitCodeSize = params.DefaultMaxInitCodeSize * 3
	cleanup := builder.Build(t)
	defer cleanup()
