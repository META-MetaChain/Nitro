...
func testBatchPosterParallel(t *testing.T, useRedis bool) {
	...
	for i := 0; i < parallelBatchPosters; i++ {
		...
		batchPoster, err := METAnode.NewBatchPoster(ctx,
			&METAnode.BatchPosterOpts{
				DataPosterDB: nil,
				...
				Config:       func() *METAnode.BatchPosterConfig { return &batchPosterConfig },
	...
	if false {
		...
		Require(t, err)
		seqInbox, err := METAnode.NewSequencerInbox(builder.L1.Client, builder.L2.ConsensusNode.DeployInfo.SequencerInbox, 0)
		Require(t, err)
