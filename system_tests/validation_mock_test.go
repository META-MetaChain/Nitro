...
func globalstateFromTestPreimages(preimages map[METAutil.PreimageType]map[common.Hash][]byte) validator.GoGlobalState {
	keccakPreimages := preimages[METAutil.Keccak256PreimageType]
...
func createMockValidationNode(t *testing.T, ctx context.Context, config *server_META.METAitratorSpawnerConfig) (*mockSpawner, *node.Node) {
	...
	if config == nil {
		config = &server_META.DefaultMETAitratorSpawnerConfig
	}
	configFetcher := func() *server_META.METAitratorSpawnerConfig { return config }
	spawner := &mockSpawner{}
...
func TestValidationServerAPI(t *testing.T) {
	...
	valInput := validator.ValidationInput{
		...
		Preimages: map[METAutil.PreimageType]map[common.Hash][]byte{
			METAutil.Keccak256PreimageType: globalstateToTestPreimages(endState),
		},
	}
...
func TestExecutionKeepAlive(t *testing.T) {
	...
	defer cancel()
	_, validationDefault := createMockValidationNode(t, ctx, nil)
	shortTimeoutConfig := server_META.DefaultMETAitratorSpawnerConfig
...
type mockBlockRecorder struct {
	...
	streamer  *METAnode.TransactionStreamer
}
...
func (m *mockBlockRecorder) MarkValid(pos METAutil.MessageIndex, resultHash common.Hash) {}
func (m *mockBlockRecorder) PrepareForRecord(ctx context.Context, start, end METAutil.MessageIndex) error {
	return nil
}

func newMockRecorder(validator *staker.StatelessBlockValidator, streamer *METAnode.TransactionStreamer) *mockBlockRecorder {
