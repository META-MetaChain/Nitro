...
func getStorageRootHash(t *testing.T, execNode *gethexec.ExecutionNode, address common.Address) common.Hash {
	t.Helper()
	statedb, err := execNode.Backend.METAInterface().BlockChain().State()
	Require(t, err)
...
func testConditionalTxThatShouldSucceed(t *testing.T, ctx context.Context, idx int, l2info info, rpcClient *rpc.Client, options *metachain_types.ConditionalOptions) {
	t.Helper()
	tx := l2info.PrepareTx("Owner", "User2", l2info.TransferGas, big.NewInt(1e12), nil)
	err := metachain.SendConditionalTransactionRPC(ctx, rpcClient, tx, options)
	if err != nil {
		Fatal(t, "SendConditionalTransactionRPC failed, idx:", idx, "err:", err)
	}
}

func testConditionalTxThatShouldFail(t *testing.T, ctx context.Context, idx int, l2info info, rpcClient *rpc.Client, options *metachain_types.ConditionalOptions, expectedErrorCode int) {
	t.Helper()
...
func getEmptyOptions(address common.Address) []*metachain_types.ConditionalOptions {
	return []*metachain_types.ConditionalOptions{
		{},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {SlotValue: map[common.Hash]common.Hash{}}}},
	}
}

func getOptions(address common.Address, rootHash common.Hash, slotValueMap map[common.Hash]common.Hash) []*metachain_types.ConditionalOptions {
	return []*metachain_types.ConditionalOptions{
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {RootHash: &rootHash}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {RootHash: &rootHash}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {SlotValue: slotValueMap}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {SlotValue: slotValueMap}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {RootHash: &rootHash}}},
		{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{address: {SlotValue: slotValueMap}}},
	}
...
func getFulfillableBlockTimeLimits(t *testing.T, blockNumber uint64, timestamp uint64) []*metachain_types.ConditionalOptions {
...
func getUnfulfillableBlockTimeLimits(t *testing.T, blockNumber uint64, timestamp uint64) []*metachain_types.ConditionalOptions {
...
func getBlockTimeLimits(t *testing.T, blockMin, blockMax math.HexOrDecimal64, timeMin, timeMax math.HexOrDecimal64) []*metachain_types.ConditionalOptions {
	basic := []*metachain_types.ConditionalOptions{
		{},
	...
	power := []*metachain_types.ConditionalOptions{
		{},
	}
	for range basic {
...
func optionsDedupProduct(t *testing.T, optionsA, optionsB []*metachain_types.ConditionalOptions) []*metachain_types.ConditionalOptions {
...
func optionsProduct(optionsA, optionsB []*metachain_types.ConditionalOptions) []*metachain_types.ConditionalOptions {
	var optionsC []*metachain_types.ConditionalOptions
	for _, a := range optionsA {
		for _, b := range optionsB {
			var c metachain_types.ConditionalOptions
			c.KnownAccounts = make(map[common.Address]metachain_types.RootHashOrSlots)
...
func dedupOptions(t *testing.T, options []*metachain_types.ConditionalOptions) []*metachain_types.ConditionalOptions {
	var result []*metachain_types.ConditionalOptions
...
func TestSendRawTransactionConditionalMultiRoutine(t *testing.T) {
	...
	for i := numTxes - 1; i >= 0; i-- {
		...
		options = append(options, &metachain_types.ConditionalOptions{KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{contractAddress: {SlotValue: map[common.Hash]common.Hash{{0}: common.BigToHash(big.NewInt(int64(expected)))}}}})
	}
	...
	for i := 0; i < len(txes); i++ {
		...
		go func() {
			...
			for ctxWithTimeout.Err() == nil {
				err := metachain.SendConditionalTransactionRPC(ctxWithTimeout, rpcClient, tx, opts)
				if err == nil {
					success <- struct{}{}
					break
				}
	...
	wg.Wait()
	bc := builder.L2.ExecNode.Backend.METAInterface().BlockChain()
	genesis := bc.Config().metachainChainParams.GenesisBlockNum
...
func TestSendRawTransactionConditionalPreCheck(t *testing.T) {
	...
	options := &metachain_types.ConditionalOptions{
		KnownAccounts: map[common.Address]metachain_types.RootHashOrSlots{
