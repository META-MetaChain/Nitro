...
func testBlockValidatorSimple(t *testing.T, dasModeString string, workloadLoops int, workload workloadType, METAitrator bool) {
	t.Parallel()
	...
	if workload == upgradeMETAOs {
		chainConfig.metachainChainParams.InitialMETAOSVersion = 10
	}
	...
	validatorConfig := METAnode.ConfigDefaultL1NonSequencerTest()
	...
	AddDefaultValNode(t, ctx, validatorConfig, !METAitrator)
	...
	if workload != upgradeMETAOs {
		...
		Require(t, err)
		if !METAmath.BigEquals(difficulty, common.Big1) {
		...
		Require(t, err)
		tx, err = METADebug.BecomeChainOwner(&auth)
		Require(t, err)
		_, err = builder.L2.EnsureTxSucceeded(tx)
		Require(t, err)
		METAOwner, err := precompilesgen.NewMETAOwner(common.HexToAddress("0x70"), builder.L2.Client)
		Require(t, err)
		tx, err = METAOwner.ScheduleMETAOSUpgrade(&auth, 11, 0)
		Require(t, err)
		_, err = builder.L2.EnsureTxSucceeded(tx)
		Require(t, err)
	...
	if !testClientB.ConsensusNode.BlockValidator.WaitForPos(t, ctx, METAutil.MessageIndex(lastBlock.NumberU64()), timeout) {
...
func TestBlockValidatorSimpleOnchainUpgradeMETAOs(t *testing.T) {
	testBlockValidatorSimple(t, "onchain", 1, upgradeMETAOs, true)
}
