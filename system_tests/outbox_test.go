...
func TestOutboxProofs(t *testing.T) {
	...
	METASysAbi, err := precompilesgen.METASysMetaData.GetAbi()
	Require(t, err, "failed to get abi")
	withdrawTopic := METASysAbi.Events["L2ToL1Tx"].ID
	merkleTopic := METASysAbi.Events["SendMerkleUpdate"].ID
	...
	METASys, err := precompilesgen.NewMETASys(types.METASysAddress, builder.L2.Client)
	Require(t, err)
	...
	for i := int64(0); i < txnCount; i++ {
		...
		tx, err := METASys.WithdrawEth(&auth, common.Address{})
		Require(t, err, "METASys failed")
		...
		if err == nil {
			merkleState, err := METASys.SendMerkleTreeState(&bind.CallOpts{})
	...
	for _, tx := range txns {
		...
		for _, log := range receipt.Logs {
			...
			if log.Topics[0] == withdrawTopic {
				parsedLog, err := METASys.ParseL2ToL1Tx(*log)
	...
	for _, root := range roots {
		...
		balanced := treeSize == METAmath.NextPowerOf2(treeSize)/2
		treeLevels := int(METAmath.Log2ceil(treeSize)) // the # of levels in the tree
