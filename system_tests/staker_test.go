...
func stakerTestImpl(t *testing.T, faultyStaker bool, honestStakerInactive bool) {
	...
	builder.L2Info = NewBlockChainTestInfo(
		t,
		types.NewmetachainSigner(types.NewLondonSigner(builder.chainConfig.ChainID)), big.NewInt(l2pricing.InitialBaseFeeWei*2),
		transferGas,
	)
	...
	config := METAnode.ConfigDefaultL1Test()
	...
	dpA, err := METAnode.StakerDataposter(ctx, rawdb.NewTable(l2nodeB.METADB, storage.StakerPrefix), l2nodeA.L1Reader, &l1authA, NewFetcherFromConfig(METAnode.ConfigDefaultL1NonSequencerTest()), nil)
	if err != nil {
	...
	Require(t, err)
	cfg := METAnode.ConfigDefaultL1NonSequencerTest()
	cfg.Staker.DataPoster.ExternalSigner = *externalSignerTestCfg(srv.address)
	dpB, err := METAnode.StakerDataposter(ctx, rawdb.NewTable(l2nodeB.METADB, storage.StakerPrefix), l2nodeB.L1Reader, &l1authB, NewFetcherFromConfig(cfg), nil)
	if err != nil {
		t.Fatalf("Error creating validator dataposter: %v", err)
	}
		...
		if err != nil && faultyStaker && i%2 == 1 {
			...
			if strings.Contains(err.Error(), "agreed with entire challenge") || strings.Contains(err.Error(), "after msg 0 expected global state") {
				...
				if !challengeMangerTimedOut {
					...
					proxyAdminSlot := common.BigToHash(METAmath.BigSub(crypto.Keccak256Hash([]byte("eip1967.proxy.admin")).Big(), common.Big1))
