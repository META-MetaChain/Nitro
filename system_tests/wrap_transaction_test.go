...
func GetPendingBlockNumber(ctx context.Context, client METAutil.L1Interface) (*big.Int, error) {
	// Attempt to get the block number from METASys, if it exists
	METASys, err := precompilesgen.NewMETASys(common.BigToAddress(big.NewInt(100)), client)
	if err != nil {
		return METAutil.GetPendingCallBlockNumber(ctx, client)
	}
	blockNum, err := METASys.METABlockNumber(&bind.CallOpts{Context: ctx})
	if err != nil {
		return METAutil.GetPendingCallBlockNumber(ctx, client)
	}
	// metachain chains don't have miners, so they're one block behind non-metachain chains.
	return blockNum.Add(blockNum, common.Big1), nil
}

// Will wait until txhash is in the blockchain and return its receipt
func WaitForTx(ctxinput context.Context, client METAutil.L1Interface, txhash common.Hash, timeout time.Duration) (*types.Receipt, error) {
...
func EnsureTxSucceeded(ctx context.Context, client METAutil.L1Interface, tx *types.Transaction) (*types.Receipt, error) {
	return EnsureTxSucceededWithTimeout(ctx, client, tx, time.Second*5)
}

func EnsureTxSucceededWithTimeout(ctx context.Context, client METAutil.L1Interface, tx *types.Transaction, timeout time.Duration) (*types.Receipt, error) {
	...
	return receipt, METAutil.DetailTxError(ctx, client, tx, receipt)
}
