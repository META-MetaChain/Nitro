...
import (
	...
	"github.com/META-MetaChain/nitro/das"
	"github.com/META-MetaChain/nitro/deploy"
	...
	"github.com/META-MetaChain/nitro/validator/valnode"
	...
	_ "github.com/META-MetaChain/nitro/nodeInterface"
	...
	"github.com/META-MetaChain/nitro/solgen/go/upgrade_executorgen"
	"github.com/META-MetaChain/nitro/statetransfer"
	"github.com/META-MetaChain/nitro/util/testhelpers"
)

type info = *BlockchainTestInfo
type client = METAutil.L1Interface

type SecondNodeParams struct {
	nodeConfig  *METAnode.Config
	execConfig  *gethexec.Config
	stackConfig *node.Config
	dasConfig   *das.DataAvailabilityConfig
	initData    *statetransfer.METAosInitializationInfo
}
...
func (b *NodeBuilder) DefaultConfig(t *testing.T, withL1 bool) *NodeBuilder {
	...
	if withL1 {
		b.isSequencer = true
		b.nodeConfig = METAnode.ConfigDefaultL1Test()
	} else {
		b.takeOwnership = true
		b.nodeConfig = METAnode.ConfigDefaultL2Test()
	}
	b.chainConfig = params.metachainDevTestChainConfig()
	b.L1Info = NewL1TestInfo(t)
	b.L2Info = NewMETATestInfo(t, b.chainConfig.ChainID)
	b.dataDir = t.TempDir()
...
func (b *NodeBuilder) Build2ndNode(t *testing.T, params *SecondNodeParams) (*TestClient, func()) {
	...
	if params.nodeConfig == nil {
		params.nodeConfig = METAnode.ConfigDefaultL1NonSequencerTest()
	}
	...
	if params.initData == nil {
		params.initData = &b.L2Info.METAInitData
	}
...
func SendSignedTxViaL1(
	...
	Require(t, err)
	txwrapped := append([]byte{METAos.L2MessageKind_SignedTx}, txbytes...)
...
type staticNodeConfigFetcher struct {
	config *METAnode.Config
}

func NewFetcherFromConfig(c *METAnode.Config) *staticNodeConfigFetcher {
	err := c.Validate()
	if err != nil {
...
func (c *staticNodeConfigFetcher) Get() *METAnode.Config {
	return c.config
}
...
func configByValidationNode(t *testing.T, clientConfig *METAnode.Config, valStack *node.Node) {
...
func AddDefaultValNode(t *testing.T, ctx context.Context, nodeConfig *METAnode.Config, useJit bool) {
...
func createTestL1BlockChainWithConfig(t *testing.T, l1info info, stackConfig *node.Config) (info, *ethclient.Client, *eth.Ethereum, *node.Node) {
	...
	chainConfig.metachainChainParams = params.metachainChainParams{}
...
func getInitMessage(ctx context.Context, t *testing.T, l1client client, addresses *chaininfo.RollupAddresses) *METAostypes.ParsedInitMessage {
	bridge, err := METAnode.NewDelayedBridge(l1client, addresses.Bridge, addresses.DeployedAt)
	Require(t, err)
	deployedAtBig := METAmath.UintToBig(addresses.DeployedAt)
...
func DeployOnTestL1(
...
) (*chaininfo.RollupAddresses, *METAostypes.ParsedInitMessage) {
	...
	l1Reader, err := headerreader.New(ctx, l1client, func() *headerreader.Config { return &headerreader.TestConfig }, METASys)
	Require(t, err)
	l1Reader.Start(ctx)
	...
	addresses, err := deploy.DeployOnL1(
		...
		0,
		METAnode.GenerateRollupConfig(false, locator.LatestWasmModuleRoot(), l1info.GetAddress("RollupOwner"), chainConfig, serializedChainConfig, common.Address{}),
		nativeToken,
		maxDataSize,
	)
	Require(t, err)
...
func createL2BlockChainWithStackConfig(
	t *testing.T, l2info *BlockchainTestInfo, dataDir string, chainConfig *params.ChainConfig, initMessage *METAostypes.ParsedInitMessage, stackConfig *node.Config, cacheConfig *gethexec.CachingConfig,
) (*BlockchainTestInfo, *node.Node, ethdb.Database, ethdb.Database, *core.BlockChain) {
	if l2info == nil {
		l2info = NewMETATestInfo(t, chainConfig.ChainID)
	}
	var stack *node.Node
	var err error
	if stackConfig == nil {
	...
	Require(t, err)
	METADb, err := stack.OpenDatabase("METAdb", 0, 0, "", false)
	Require(t, err)

	initReader := statetransfer.NewMemoryInitDataReader(&l2info.METAInitData)
	if initMessage == nil {
		serializedChainConfig, err := json.Marshal(chainConfig)
		Require(t, err)
	...
	return l2info, stack, chainDb, METADb, blockchain
}
...
// Create and deploy L1 and METAnode for L2
func createTestNodeWithL1(
	...
	l2info info, currentNode *METAnode.Node, l2client *ethclient.Client, l2stack *node.Node,
	l1info info, l1backend *eth.Ethereum, l1client *ethclient.Client, l1stack *node.Node,
) {
	if nodeConfig == nil {
		nodeConfig = METAnode.ConfigDefaultL1Test()
	}
	if execConfig == nil {
	...
	_, l2stack, l2chainDb, l2METADb, l2blockchain = createL2BlockChainWithStackConfig(t, l2info, "", chainConfig, initMessage, stackConfig, &execConfig.Caching)
	...
	currentNode, err = METAnode.CreateNode(
		ctx, l2stack, execNode, l2METADb, NewFetcherFromConfig(nodeConfig), l2blockchain.Config(), l1client,
...
func createTestNode(
	t *testing.T, ctx context.Context, l2Info *BlockchainTestInfo, nodeConfig *METAnode.Config, execConfig *gethexec.Config, chainConfig *params.ChainConfig, takeOwnership bool,
) (*BlockchainTestInfo, *METAnode.Node, *ethclient.Client) {
	if nodeConfig == nil {
		nodeConfig = METAnode.ConfigDefaultL2Test()
	}
	if execConfig == nil {
		execConfig = gethexec.ConfigDefaultTest()
	}
	...
	l2info, stack, chainDb, METADb, blockchain := createL2BlockChain(t, l2Info, "", chainConfig, &execConfig.Caching)
	...
	currentNode, err := METAnode.CreateNode(ctx, stack, execNode, METADb, NewFetcherFromConfig(nodeConfig), blockchain.Config(), nil, nil, nil, nil, nil, feedErrChan)
	Require(t, err)
	...
	if takeOwnership {
		...
		METAdebug, err := precompilesgen.NewMETADebug(common.HexToAddress("0xff"), client)
		Require(t, err, "failed to deploy METADebug")

		tx, err := METAdebug.BecomeChainOwner(&debugAuth)
		Require(t, err, "failed to deploy METADebug")
...
func StartWatchChanErr(t *testing.T, ctx context.Context, feedErrChan chan error, node *METAnode.Node) {
	go func() {
		select {
		case <-ctx.Done():
			return
...
func Create2ndNodeWithConfig(
	...
	l1stack *node.Node,
	l1info *BlockchainTestInfo,
	l2InitData *statetransfer.METAosInitializationInfo,
	nodeConfig *METAnode.Config,
	execConfig *gethexec.Config,
	stackConfig *node.Config,
) (*ethclient.Client, *METAnode.Node) {
	if nodeConfig == nil {
		nodeConfig = METAnode.ConfigDefaultL1NonSequencerTest()
	}
	if execConfig == nil {
	...
	Require(t, err)
	l2METADb, err := l2stack.OpenDatabase("METAdb", 0, 0, "", false)
	Require(t, err)
	...
	chainConfig := firstExec.METAInterface.BlockChain().Config()
	...
	currentNode, err := METAnode.CreateNode(ctx, l2stack, currentExec, l2METADb, NewFetcherFromConfig(nodeConfig), l2blockchain.Config(), l1client, first.DeployInfo, &txOpts, &txOpts, dataSigner, feedErrChan)
	Require(t, err)
...
func setupConfigWithDAS(
...
) (*params.ChainConfig, *METAnode.Config, *das.LifecycleManager, string, *blsSignatures.PublicKey) {
	l1NodeConfigA := METAnode.ConfigDefaultL1Test()
	chainConfig := params.metachainDevTestChainConfig()
	var dbPath string
	var err error
	...
	case "db":
		...
		chainConfig = params.metachainDevTestDASChainConfig()
	case "files":
...
func getExecNode(t *testing.T, node *METAnode.Node) *gethexec.ExecutionNode {
	t.Helper()
	gethExec, ok := node.Execution.(*gethexec.ExecutionNode)
	if !ok {
		t.Fatal("failed to get exec node from METAnode")
	}
	return gethExec
}
