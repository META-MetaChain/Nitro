...
func TestContractTxDeploy(t *testing.T) {
	...
	for stateNonce := uint64(0); stateNonce < 2; stateNonce++ {
		...
		contractTx := &types.metachainContractTx{
			ChainId:   params.metachainDevTestChainConfig().ChainID,
			RequestId: requestId,
			From:      from,
		...
		l2Msg := []byte{METAos.L2MessageKind_ContractTx}
		l2Msg = append(l2Msg, METAmath.Uint64ToU256Bytes(contractTx.Gas)...)
		l2Msg = append(l2Msg, METAmath.U256Bytes(contractTx.GasFeeCap)...)
		l2Msg = append(l2Msg, common.Hash{}.Bytes()...) // to is zero, translated into nil
		l2Msg = append(l2Msg, METAmath.U256Bytes(contractTx.Value)...)
		l2Msg = append(l2Msg, contractTx.Data...)

		err = builder.L2.ConsensusNode.TxStreamer.AddMessages(pos, true, []METAostypes.MessageWithMetadata{
			{
