...
func TestDASRekey(t *testing.T) {
	...
	l1NodeConfigB := METAnode.ConfigDefaultL1NonSequencerTest()
...

	{
		...
		// Setup L2 chain
		_, l2stackA, l2chainDb, l2METADb, l2blockchain := createL2BlockChainWithStackConfig(t, l2info, nodeDir, chainConfig, initMessage, nil, nil)
		...
		nodeA, err := METAnode.CreateNode(ctx, l2stackA, execA, l2METADb, NewFetcherFromConfig(l1NodeConfigA), l2blockchain.Config(), l1client, addresses, sequencerTxOptsPtr, sequencerTxOptsPtr, nil, feedErrChan)
		Require(t, err)
		...
		l2clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2info.METAInitData, l1NodeConfigB, nil, nil)
	...
	l2METADb, err := l2stackA.OpenDatabase("METAdb", 0, 0, "", false)
	Require(t, err)
...
func TestDASComplexConfigAndRestMirror(t *testing.T) {
	...
	// Setup L2 chain
	l2info, l2stackA, l2chainDb, l2METADb, l2blockchain := createL2BlockChainWithStackConfig(t, nil, "", chainConfig, initMessage, nil, nil)
	l2info.GenerateAccount("User2")
	...
	sequencerTxOpts := l1info.GetDefaultTransactOpts("Sequencer", ctx)
	sequencerTxOptsPtr := &sequencerTxOpts
	nodeA, err := METAnode.CreateNode(ctx, l2stackA, execA, l2METADb, NewFetcherFromConfig(l1NodeConfigA), l2blockchain.Config(), l1client, addresses, sequencerTxOptsPtr, sequencerTxOptsPtr, dataSigner, feedErrChan)
	Require(t, err)
	Require(t, nodeA.Start(ctx))
	l2clientA := ClientForStack(t, l2stackA)
