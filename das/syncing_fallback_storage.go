...
type l1SyncService struct {
	...
	dataSource METAstate.DataAvailabilityReader
...
func newl1SyncService(config *SyncToStorageConfig, syncTo StorageService, dataSource METAstate.DataAvailabilityReader, l1Reader *headerreader.HeaderReader, inboxAddr common.Address) (*l1SyncService, error) {
...
func (s *l1SyncService) processBatchDelivered(ctx context.Context, batchDeliveredLog types.Log) error {
	...
	storeUntil := METAmath.SaturatingUAdd(deliveredEvent.TimeBounds.MaxTimestamp, uint64(s.config.RetentionPeriod.Seconds()))
	...
	preimages := make(map[METAutil.PreimageType]map[common.Hash][]byte)
	if _, err = METAstate.RecoverPayloadFromDasBatch(ctx, deliveredEvent.BatchSequenceNumber.Uint64(), data, s.dataSource, preimages, METAstate.KeysetValidate); err != nil {
...
func FindDASDataFromLog(
	...
	if deliveredEvent.DataLocation == uint8(batchDataSeparateEvent) {
		...
		txData, err := METAutil.GetLogEmitterTxData(ctx, l1Client, batchDeliveredLog)
		if err != nil {
			return nil, err
		}
	...
	if !METAstate.IsDASMessageHeaderByte(data[0]) {
...
func NewSyncingFallbackStorageService(ctx context.Context,
	...
	backup METAstate.DataAvailabilityReader,
