...
func (c *RestfulDasClient) GetByHash(ctx context.Context, hash common.Hash) ([]byte, error) {
	...
	if !dastree.ValidHash(hash, decodedBytes) {
		return nil, METAstate.ErrHashMismatch
	}
...
func (c *RestfulDasClient) ExpirationPolicy(ctx context.Context) (METAstate.ExpirationPolicy, error) {
	...
	return METAstate.StringToExpirationPolicy(response.ExpirationPolicy)
}
