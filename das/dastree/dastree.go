...
func RecordHash(record func(bytes32, []byte), preimage ...[]byte) bytes32 {
	...
	unrolled := METAmath.ConcatByteSlices(preimage...)
	if len(unrolled) == 0 {
		return METAmath.FlipBit(keccord(prepend(LeafByte, keccord([]byte{}).Bytes())), 0)
	}

	length := uint32(len(unrolled))
	leaves := []node{}
	for bin := uint32(0); bin < length; bin += BinSize {
		end := METAmath.MinInt(bin+BinSize, length)
	...
	for len(layer) > 1 {
		...
		for i := 0; i < prior-1; i += 2 {
			...
			dataUnder := METAmath.ConcatByteSlices(firstHash, otherHash, METAmath.Uint32ToBytes(sizeUnder))
			parent := node{
	...
	return METAmath.FlipBit(layer[0].hash, 0)
}
...
func FlatHashToTreeHash(flat bytes32) bytes32 {
	...
	return METAmath.FlipBit(crypto.Keccak256Hash(FlatHashToTreeLeaf(flat)), 0)
}
...
func Content(root bytes32, oracle func(bytes32) ([]byte, error)) ([]byte, error) {
	...
	for len(stack) > 0 {
		...
		case NodeByte:
			...
			power := uint32(METAmath.NextOrCurrentPowerOf2(uint64(count)))
