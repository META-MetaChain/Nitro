...
func NewRestfulClientAggregator(ctx context.Context, config *RestfulClientAggregatorConfig) (*SimpleDASReaderAggregator, error) {
	a := SimpleDASReaderAggregator{
		config: config,
		stats:  make(map[METAstate.DataAvailabilityReader]readerStats),
	}
...
type readerStatMessage struct {
	readerStat
	reader METAstate.DataAvailabilityReader
}
	...
	go func() {
		...
		for readers := si.nextReaders(); len(readers) != 0 && subCtx.Err() == nil; readers = si.nextReaders() {
			...
			for _, reader := range readers {
				wg.Add(1)
				go func(reader METAstate.DataAvailabilityReader) {
					defer wg.Done()
...
func (a *SimpleDASReaderAggregator) tryGetByHash(
	ctx context.Context, hash common.Hash, reader METAstate.DataAvailabilityReader,
) ([]byte, error) {
...
func (a *SimpleDASReaderAggregator) Start(ctx context.Context) {
	...
	updateRestfulDasClients := func(urls []string) {
		...
		combinedReaders := make(map[METAstate.DataAvailabilityReader]bool)
		...
		a.readers = make([]METAstate.DataAvailabilityReader, 0, len(combinedUrls))
...
func (a *SimpleDASReaderAggregator) ExpirationPolicy(ctx context.Context) (METAstate.ExpirationPolicy, error) {
	a.readersMutex.RLock()
	defer a.readersMutex.RUnlock()
	if len(a.readers) == 0 {
	...
	for _, serv := range a.readers {
		...
		if ep != expectedExpirationPolicy {
			return METAstate.MixedTimeout, nil
		}
	}
