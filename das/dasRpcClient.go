...
func (c *DASRPCClient) Store(ctx context.Context, message []byte, timeout uint64, reqSig []byte) (*METAstate.DataAvailabilityCertificate, error) {
	...
	return &METAstate.DataAvailabilityCertificate{
...
func (c *DASRPCClient) ExpirationPolicy(ctx context.Context) (METAstate.ExpirationPolicy, error) {
	var res string
	err := c.clnt.CallContext(ctx, &res, "das_expirationPolicy")
	if err != nil {
		return -1, err
	}
	return METAstate.StringToExpirationPolicy(res)
}
