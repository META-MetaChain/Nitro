...
func (r *RedundantStorageService) ExpirationPolicy(ctx context.Context) (METAstate.ExpirationPolicy, error) {
	...
	var res METAstate.ExpirationPolicy = -1
	for _, serv := range r.innerServices {
		...
		case METAstate.KeepForever:
			return METAstate.KeepForever, nil
		case METAstate.DiscardAfterArchiveTimeout:
			res = METAstate.DiscardAfterArchiveTimeout
		case METAstate.DiscardAfterDataTimeout:
			if res != METAstate.DiscardAfterArchiveTimeout {
				res = METAstate.DiscardAfterDataTimeout
			}
		}
	}
	if res == -1 {
