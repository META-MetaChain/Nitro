...
func (a *Aggregator) Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*METAstate.DataAvailabilityCertificate, error) {
	...
	for _, d := range a.services {
		go func(ctx context.Context, d ServiceDetails) {
			...
			const metricBase string = "META/das/rpc/aggregator/store"
	...
	var aggCert METAstate.DataAvailabilityCertificate
