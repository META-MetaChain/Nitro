...
type DataAvailabilityServiceWriter interface {
	...
	Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*METAstate.DataAvailabilityCertificate, error)
	fmt.Stringer
}
...
func Serialize(c *METAstate.DataAvailabilityCertificate) []byte {

	flags := METAstate.DASMessageHeaderFlag
	if c.Version != 0 {
		flags |= METAstate.TreeDASMessageHeaderFlag
	}
