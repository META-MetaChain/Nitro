...
func expectedResultsForL1Test(input *l1PricingTest) *l1TestExpectedResults {
	...
	availableFunds := METAmath.UintToBig(3 * input.fundsCollectedPerSecond)
	uncappedAvailableFunds := availableFunds
	if input.amortizationCapBips != 0 {
		availableFundsCap := METAmath.BigMulByBips(METAmath.BigMulByUint(
			METAmath.UintToBig(input.unitsPerSecond),
			input.l1BasefeeGwei*params.GWei),
			METAmath.SaturatingCastToBips(input.amortizationCapBips),
		)
		if METAmath.BigLessThan(availableFundsCap, availableFunds) {
			availableFunds = availableFundsCap
		}
	}
	fundsWantedForRewards := big.NewInt(int64(input.unitReward * input.unitsPerSecond))
	unitsAllocated := METAmath.UintToBig(input.unitsPerSecond)
	if METAmath.BigLessThan(availableFunds, fundsWantedForRewards) {
		ret.rewardRecipientBalance = availableFunds
	} else {
		ret.rewardRecipientBalance = fundsWantedForRewards
	}
	availableFunds = METAmath.BigSub(availableFunds, ret.rewardRecipientBalance)
	uncappedAvailableFunds = METAmath.BigSub(uncappedAvailableFunds, ret.rewardRecipientBalance)
	ret.unitsRemaining = (3 * input.unitsPerSecond) - unitsAllocated.Uint64()

	maxCollectable := big.NewInt(int64(input.fundsSpent))
	if METAmath.BigLessThan(availableFunds, maxCollectable) {
		maxCollectable = availableFunds
	}
	ret.fundsReceived = maxCollectable
	uncappedAvailableFunds = METAmath.BigSub(uncappedAvailableFunds, maxCollectable)
...
func _testL1PricingFundsDue(t *testing.T, testParams *l1PricingTest, expectedResults *l1TestExpectedResults) {
	...
	METAosSt, err := METAosState.OpenMETAosState(evm.StateDB, burner)
	Require(t, err)

	l1p := METAosSt.L1PricingState()
	...
	Require(t, err)
	version := METAosSt.METAOSVersion()
	scenario := util.TracingDuringEVM
	err = l1p.UpdateForBatchPosterSpending(
		evm.StateDB, evm, version, 1, 3, firstPoster, METAmath.UintToBig(testParams.fundsSpent), METAmath.UintToBig(testParams.l1BasefeeGwei*params.GWei), scenario,
	)
	Require(t, err)
	rewardRecipientBalance := evm.StateDB.GetBalance(rewardAddress)
	if !METAmath.BigEquals(rewardRecipientBalance, expectedResults.rewardRecipientBalance) {
	...
	if !METAmath.BigEquals(fundsReceived, expectedResults.fundsReceived) {
	...
	if !METAmath.BigEquals(fundsStillHeld, expectedResults.fundsStillHeld) {
...
func TestUpdateTimeUpgradeBehavior(t *testing.T) {
	...
	l1p := METAosSt.L1PricingState()
	amount := METAmath.UintToBig(10 * params.GWei)
...
func _testL1PriceEquilibration(t *testing.T, initialL1BasefeeEstimate *big.Int, equilibriumL1BasefeeEstimate *big.Int) {
	evm := newMockEVMForTesting()
	stateDb := evm.StateDB
	state, err := METAosState.OpenMETAosState(stateDb, burn.NewSystemBurner(nil, false))
	Require(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		Require(t, err)
		feesToAdd := METAmath.BigMulByUint(currentPricePerUnit, unitsToAdd)
		...
		err = l1p.UpdateForBatchPosterSpending(
			...
			uint64(10*(i+1)+5),
			bpAddr,
			METAmath.BigMulByUint(equilibriumL1BasefeeEstimate, unitsToAdd),
			equilibriumL1BasefeeEstimate,
			util.TracingBeforeEVM,
		)
		Require(t, err)
	}
	expectedMovement := METAmath.BigSub(equilibriumL1BasefeeEstimate, initialL1BasefeeEstimate)
	actualPricePerUnit, err := l1p.PricePerUnit()
	Require(t, err)
	actualMovement := METAmath.BigSub(actualPricePerUnit, initialL1BasefeeEstimate)
...
func _withinOnePercent(v1, v2 *big.Int) bool {
	if METAmath.BigMulByUint(v1, 100).Cmp(METAmath.BigMulByUint(v2, 101)) > 0 {
		return false
	}
	if METAmath.BigMulByUint(v2, 100).Cmp(METAmath.BigMulByUint(v1, 101)) > 0 {
		return false
	}
	return true
}
...
func newMockEVMForTesting() *vm.EVM {
	...
	_, statedb := METAosState.NewMETAosMemoryBackedMETAOSState()
