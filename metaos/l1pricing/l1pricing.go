...
import (
	...
	am "github.com/META-MetaChain/nitro/util/METAmath"
...
type L1PricingState struct {
	...
	lastSurplus          storage.StorageBackedBigInt  // introduced in METAOS version 2
	perBatchGasCost      storage.StorageBackedInt64   // introduced in METAOS version 3
	amortizedCostCapBips storage.StorageBackedUint64  // in basis points; introduced in METAOS version 3
...
var InitialEquilibrationUnitsV0 = METAmath.UintToBig(60 * params.TxDataNonZeroGasEIP2028 * 100000)
var InitialEquilibrationUnitsV6 = METAmath.UintToBig(params.TxDataNonZeroGasEIP2028 * 10000000)
...
func (ps *L1PricingState) SetLastSurplus(val *big.Int, METAosVersion uint64) error {
	if METAosVersion < 7 {
...
func (ps *L1PricingState) UpdateForBatchPosterSpending(
	...
	if METAosVersion < 10 {
		return ps._preversion10_UpdateForBatchPosterSpending(statedb, evm, METAosVersion, updateTime, currentTime, batchPoster, weiSpent, l1Basefee, scenario)
	}
	...
	if unitsAllocated > 0 {
		...
		if err := ps.SetLastSurplus(surplus, METAosVersion); err != nil {
			return err
		}
...
var randV = METAmath.BigMulByUint(params.metachainOneChainConfig().ChainID, 3)
...
func (ps *L1PricingState) PosterDataCost(message *core.Message, poster common.Address, brotliCompressionLevel uint64) (*big.Int, uint64) {
	...
	units := ps.getPosterUnitsWithoutCache(tx, poster, brotliCompressionLevel)
	units = METAmath.UintMulByBips(units+estimationPaddingUnits, METAmath.OneInBips+estimationPaddingBasisPoints)
	pricePerUnit, _ := ps.PricePerUnit()
	return am.BigMulByUint(pricePerUnit, units), units
}

func byteCountAfterBrotliLevel(input []byte, level int) (uint64, error) {
	compressed, err := METAcompress.CompressLevel(input, level)
	if err != nil {
		return 0, err
	}
