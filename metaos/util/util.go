...
var PackMETARetryableTxRedeem func(...interface{}) ([]byte, error)
...
func init() {
	...
	InverseAddressAliasOffset = METAmath.BigSub(new(big.Int).Lsh(big.NewInt(1), 160), AddressAliasOffset)
	...
	ParseRedeemScheduledLog = logParser(precompilesgen.METARetryableTxABI, "RedeemScheduled")
	ParseL2ToL1TxLog = logParser(precompilesgen.METASysABI, "L2ToL1Tx")
	ParseL2ToL1TransactionLog = logParser(precompilesgen.METASysABI, "L2ToL1Transaction")

	acts := precompilesgen.METAosActsABI
	...
	PackMETARetryableTxRedeem, _ = callParser(precompilesgen.METARetryableTxABI, "redeem")
}
...
func DoesTxTypeAlias(txType *byte) bool {
	...
	switch *txType {
	case types.metachainUnsignedTxType:
		fallthrough
	case types.metachainContractTxType:
		fallthrough
	case types.metachainRetryTxType:
		return true
	}
	return false
}
...
func TxTypeHasPosterCosts(txType byte) bool {
	...
	case types.metachainRetryTxType:
		fallthrough
	case types.metachainInternalTxType:
		fallthrough
	case types.metachainSubmitRetryableTxType:
		return false
	}
	return true
}
