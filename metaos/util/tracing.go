...
func (info *TracingInfo) RecordStorageGet(key common.Hash) {
	...
	if info.Scenario == TracingDuringEVM {
		...
		tracer.CapturemetachainStorageGet(key, info.Depth, info.Scenario == TracingBeforeEVM)
	}
}
...
func (info *TracingInfo) RecordStorageSet(key, value common.Hash) {
	...
	if info.Scenario == TracingDuringEVM {
		...
		tracer.CapturemetachainStorageSet(key, value, info.Depth, info.Scenario == TracingBeforeEVM)
	}
}
