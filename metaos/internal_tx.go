...
func InternalTxStartBlock(
	...
	return &types.metachainInternalTx{
		ChainId: chainId,
		Data:    data,
	}
}

func ApplyInternalTxUpdate(tx *types.metachainInternalTx, state *METAosState.METAosState, evm *vm.EVM) error {
	if len(tx.Data) < 4 {
	...
	case InternalTxStartBlockMethodID:
		...
		if l1BlockNumber > oldL1BlockNumber {
			...
			state.Restrict(state.Blockhashes().RecordNewL1Block(l1BlockNumber-1, prevHash, state.METAOSVersion()))
		}
		...
		return state.UpgradeMETAosVersionIfNecessary(currentTime, evm.StateDB, evm.ChainConfig())
	case InternalTxBatchPostingReportMethodID:
		...
		gasSpent := METAmath.SaturatingAdd(perBatchGas, METAmath.SaturatingCast(batchDataGas))
		weiSpent := METAmath.BigMulByUint(l1BaseFeeWei, METAmath.SaturatingUCast(gasSpent))
