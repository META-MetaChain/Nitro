...
func MakeGenesisBlock(parentHash common.Hash, blockNumber uint64, timestamp uint64, stateRoot common.Hash, chainConfig *params.ChainConfig) *types.Block {
	...
	genesisHeaderInfo := types.HeaderInfo{
		...
		SendCount:          0,
		L1BlockNumber:      0,
		METAOSFormatVersion: chainConfig.metachainChainParams.InitialMETAOSVersion,
	}
...
func InitializeMETAosInDatabase(db ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, initMessage *METAostypes.ParsedInitMessage, timestamp uint64, accountsPerSync uint) (common.Hash, error) {
	...
	commit := func() (common.Hash, error) {
		root, err := statedb.Commit(chainConfig.metachainChainParams.GenesisBlockNum, true)
		if err != nil {
	...
	METAosState, err := InitializeMETAosState(statedb, burner, chainConfig, initMessage)
	if err != nil {
		log.Crit("failed to open the METAOS state", "error", err)
	}

	addrTable := METAosState.AddressTable()
	...
	err = initializeRetryables(statedb, METAosState.RetryableState(), retryableReader, timestamp)
	if err != nil {
		return common.Hash{}, err
	}
	...
	for accountDataReader.More() {
		...
		err = initializeMETAosAccount(statedb, METAosState, *account)
		if err != nil {
			return common.Hash{}, err
		}
...
func initializeRetryables(statedb *state.StateDB, rs *retryables.RetryableState, initData statetransfer.RetryableDataReader, currentTimestamp uint64) error {
	...
	sort.Slice(retryablesList, func(i, j int) bool {
		...
		if a.Timeout == b.Timeout {
			return METAmath.BigLessThan(a.Id.Big(), b.Id.Big())
		}
...
func initializeMETAosAccount(_ *state.StateDB, METAosState *METAosState, account statetransfer.AccountInitializationInfo) error {
	l1pState := METAosState.L1PricingState()
