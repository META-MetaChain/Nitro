...
func TestJsonMarshalUnmarshal(t *testing.T) {
	...
	tryMarshalUnmarshal(
		&statetransfer.METAosInitializationInfo{
...
func tryMarshalUnmarshal(input *statetransfer.METAosInitializationInfo, t *testing.T) {
	...
	output := statetransfer.METAosInitializationInfo{}
	...
	var initData statetransfer.METAosInitializationInfo
	...
	stateroot, err := InitializeMETAosInDatabase(raw, initReader, chainConfig, METAostypes.TestInitMessage, 0, 0)
	Require(t, err)

	stateDb, err := state.New(stateroot, state.NewDatabase(raw), nil)
	Require(t, err)

	METAState, err := OpenMETAosState(stateDb, &burn.SystemBurner{})
	Require(t, err)
	checkAddressTable(METAState, input.AddressTableContents, t)
	checkRetryables(METAState, input.RetryableData, t)
	checkAccounts(stateDb, METAState, input.Accounts, t)
}
...
func checkAddressTable(METAState *METAosState, addrTable []common.Address, t *testing.T) {
	atab := METAState.AddressTable()
...
func checkRetryables(METAState *METAosState, expected []statetransfer.InitializationDataForRetryable, t *testing.T) {
	ret := METAState.RetryableState()
...
func checkAccounts(db *state.StateDB, METAState *METAosState, accts []statetransfer.AccountInitializationInfo, t *testing.T) {
	l1p := METAState.L1PricingState()
