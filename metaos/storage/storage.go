...
// Storage allows METAOS to store data persistently in the Ethereum-compatible stateDB. This is represented in
...
func NewGeth(statedb vm.StateDB, burner burn.Burner) *Storage {
	...
	statedb.SetNonce(account, 1) // setting the nonce ensures Geth won't treat METAOS as empty
	return &Storage{
		account:    account,
		db:         statedb,
		storageKey: []byte{},
...
func (s *Storage) CleMETAyUint64(key uint64) error {
...
func (s *Storage) CleMETAytes() error {
	bytesLeft, err := s.GetUint64ByUint64(0)
	if err != nil {
		return err
	}
...
func (s *Storage) Keccak(data ...[]byte) ([]byte, error) {
	...
	cost := 30 + 6*METAmath.WordsForBytes(uint64(byteCount))
...
func (sbu *StorageBackedBips) Get() (METAmath.Bips, error) {
	value, err := sbu.backing.Get()
	return METAmath.Bips(value), err
}

func (sbu *StorageBackedBips) Set(bips METAmath.Bips) error {
...
func (sbbu *StorageBackedBigUint) SetSaturatingWithWarning(val *big.Int, name string) error {
	if val.Sign() < 0 {
		log.Warn("METAOS storage big uint underflowed", "name", name, "value", val)
		val = common.Big0
	} else if val.BitLen() > 256 {
		log.Warn("METAOS storage big uint overflowed", "name", name, "value", val)
...
func (sbbi *StorageBackedBigInt) SetSaturatingWithWarning(val *big.Int, name string) error {
	if val.Sign() < 0 {
		...
		val = new(big.Int).Add(val, twoToThe256)
		if val.BitLen() < 256 || val.Sign() <= 0 { // require that it's positive and the top bit is set
			log.Warn("METAOS storage big uint underflowed", "name", name, "value", origVal)
			val.Set(twoToThe255)
		}
	} else if val.BitLen() >= 256 {
		log.Warn("METAOS storage big uint overflowed", "name", name, "value", val)
...
func (sbb *StorageBackedBytes) Clear() error {
	return sbb.Storage.CleMETAytes()
}
