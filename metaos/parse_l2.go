...
func ParseL2Transactions(msg *METAostypes.L1IncomingMessage, chainId *big.Int, batchFetcher InfallibleBatchFetcher) (types.Transactions, error) {
	if len(msg.L2msg) > METAostypes.MaxL2MessageSize {
		// ignore the message if l2msg is too large
		return nil, errors.New("message too large")
	}
	switch msg.Header.Kind {
	case METAostypes.L1MessageType_L2Message:
		return parseL2Message(bytes.NewReader(msg.L2msg), msg.Header.Poster, msg.Header.Timestamp, msg.Header.RequestId, chainId, 0)
	case METAostypes.L1MessageType_Initialize:
		return nil, errors.New("ParseL2Transactions encounted initialize message (should've been handled explicitly at genesis)")
	case METAostypes.L1MessageType_EndOfBlock:
		return nil, nil
	case METAostypes.L1MessageType_L2FundedByL1:
		if len(msg.L2msg) < 1 {
		...
		kind := msg.L2msg[0]
		depositRequestId := crypto.Keccak256Hash(msg.Header.RequestId[:], METAmath.U256Bytes(common.Big0))
		unsignedRequestId := crypto.Keccak256Hash(msg.Header.RequestId[:], METAmath.U256Bytes(common.Big1))
		tx, err := parseUnsignedTx(bytes.NewReader(msg.L2msg[1:]), msg.Header.Poster, &unsignedRequestId, chainId, kind)
		if err != nil {
			return nil, err
		}
		deposit := types.NewTx(&types.metachainDepositTx{
			ChainId:     chainId,
...
func parseL2Message(rd io.Reader, poster common.Address, timestamp uint64, requestId *common.Hash, chainId *big.Int, depth int) (types.Transactions, error) {
	...
	case L2MessageKind_Batch:
		...
		index := big.NewInt(0)
		for {
			nextMsg, err := util.BytestringFromReader(rd, METAostypes.MaxL2MessageSize)
			if err != nil {
			...
			if requestId != nil {
				subRequestId := crypto.Keccak256Hash(requestId[:], METAmath.U256Bytes(index))
	...
	case L2MessageKind_SignedTx:
		...
		if newTx.Type() >= types.metachainDepositTxType || newTx.Type() == types.BlobTxType {
...
func parseUnsignedTx(rd io.Reader, poster common.Address, requestId *common.Hash, chainId *big.Int, txKind byte) (*types.Transaction, error) {
	...
	switch txKind {
	case L2MessageKind_UnsignedUserTx:
		inner = &types.metachainUnsignedTx{
			ChainId:   chainId,
			From:      poster,
			Nonce:     nonce,
	...
	case L2MessageKind_ContractTx:
		...
		inner = &types.metachainContractTx{
			ChainId:   chainId,
			RequestId: *requestId,
			From:      poster,
			GasFeeCap: maxFeePerGas.Big(),
...
func parseEthDepositMessage(rd io.Reader, header *METAostypes.L1IncomingMessageHeader, chainId *big.Int) (*types.Transaction, error) {
...
func parseSubmitRetryableMessage(rd io.Reader, header *METAostypes.L1IncomingMessageHeader, chainId *big.Int) (*types.Transaction, error) {
	...
	if dataLength > METAostypes.MaxL2MessageSize {
	...
	tx := &types.metachainSubmitRetryableTx{
		ChainId:          chainId,
...
func parseBatchPostingReportMessage(rd io.Reader, chainId *big.Int, msgBatchGasCost *uint64, batchFetcher InfallibleBatchFetcher) (*types.Transaction, error) {
	batchTimestamp, batchPosterAddr, batchHash, batchNum, l1BaseFee, extraGas, err := METAostypes.ParseBatchPostingReportMessageFields(rd)
	if err != nil {
		return nil, err
	}
	...
	if msgBatchGasCost != nil {
		...
		batchDataGas = METAostypes.ComputeBatchGasCost(batchData)
	}
	batchDataGas = METAmath.SaturatingUAdd(batchDataGas, extraGas)
	...
	return types.NewTx(&types.metachainInternalTx{
		ChainId: chainId,
		Data:    data,
