...
func (rs *RetryableState) RetryableSizeBytes(id common.Hash, currentTime uint64) (uint64, error) {
	...
	calldata := 32 + 32*METAmath.WordsForBytes(size) // length + contents
...
func (rs *RetryableState) DeleteRetryable(id common.Hash, evm *vm.EVM, scenario util.TracingScenario) (bool, error) {
	...
	// we ignore returned error as we expect that if one CleMETAyUint64 fails, than all consecutive calls to CleMETAyUint64 will fail with the same error (not modifying state), and then CleMETAytes will also fail with the same error (also not modifying state) - and this one we check and return
	_ = retStorage.CleMETAyUint64(numTriesOffset)
	_ = retStorage.CleMETAyUint64(fromOffset)
	_ = retStorage.CleMETAyUint64(toOffset)
	_ = retStorage.CleMETAyUint64(callvalueOffset)
	_ = retStorage.CleMETAyUint64(beneficiaryOffset)
	_ = retStorage.CleMETAyUint64(timeoutOffset)
	_ = retStorage.CleMETAyUint64(timeoutWindowsLeftOffset)
	err = retStorage.OpenSubStorage(calldataKey).CleMETAytes()
	return true, err
}
...
func (retryable *Retryable) MakeTx(chainId *big.Int, nonce uint64, gasFeeCap *big.Int, gas uint64, ticketId common.Hash, refundTo common.Address, maxRefund *big.Int, submissionFeeRefund *big.Int) (*types.metachainRetryTx, error) {
...
func RetryableSubmissionFee(calldataLengthInBytes int, l1BaseFee *big.Int) *big.Int {
	return METAmath.BigMulByUint(l1BaseFee, uint64(1400+6*calldataLengthInBytes))
}
