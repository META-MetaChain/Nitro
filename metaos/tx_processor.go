...
var METAosAddress = types.METAosAddress

const GasEstimationL1PricePadding METAmath.Bips = 11000 // pad estimates by 10%

// A TxProcessor is created and freed for every L2 transaction.
// It tracks state for METAOS, allowing it infuence in Geth's tx processing.
// Public fields are accessible in precompiles.
type TxProcessor struct {
	msg              *core.Message
	state            *METAosState.METAosState
...
func NewTxProcessor(evm *vm.EVM, msg *core.Message) *TxProcessor {
	tracingInfo := util.NewTracingInfo(evm, msg.From, METAosAddress, util.TracingBeforeEVM)
	METAosState := METAosState.OpenSystemMETAosStateOrPanic(evm.StateDB, tracingInfo, false)
	return &TxProcessor{
		msg:                 msg,
		state:               METAosState,
...
func takeFunds(pool *big.Int, take *big.Int) *big.Int {
	...
	if METAmath.BigLessThan(pool, take) {
...
func (p *TxProcessor) StartTxHook() (endTxNow bool, gasUsed uint64, err error, returnData []byte) {
	...
	startTracer := func() func() {
		...
		p.state = METAosState.OpenSystemMETAosStateOrPanic(evm.StateDB, tracingInfo, false)
	...
	case *types.metachainInternalTx:
		defer (startTracer())()
		if p.msg.From != METAosAddress {
			return false, 0, errors.New("internal tx not from METAAddress"), nil
		}
	...
	case *types.metachainSubmitRetryableTx:
		...
		if METAmath.BigLessThan(tx.MaxSubmissionFee, submissionFee) {
		...
		submissionFeeRefund := takeFunds(availableRefund, METAmath.BigSub(tx.MaxSubmissionFee, submissionFee))
		...
		maxGasCost := METAmath.BigMulByUint(tx.GasFeeCap, usergas)
		maxFeePerGasTooLow := METAmath.BigLessThan(tx.GasFeeCap, effectiveBaseFee)
		if METAmath.BigLessThan(balance, maxGasCost) || usergas < params.TxGas || maxFeePerGasTooLow {
		...
		gascost := METAmath.BigMulByUint(effectiveBaseFee, usergas)
		networkCost := gascost
		if p.state.METAOSVersion() >= 11 {
			...
			if infraFeeAccount != (common.Address{}) {
				...
				p.state.Restrict(err)
				infraFee := METAmath.BigMin(minBaseFee, effectiveBaseFee)
				infraCost := METAmath.BigMulByUint(infraFee, usergas)
		...
		if METAmath.BigGreaterThan(networkCost, common.Big0) {
		...
		gasPriceRefund := METAmath.BigMulByUint(METAmath.BigSub(tx.GasFeeCap, effectiveBaseFee), tx.Gas)
		...
		if tracer := evm.Config.Tracer; tracer != nil {
			redeem, err := util.PackMETARetryableTxRedeem(ticketId)
			if err == nil {
				tracingInfo.MockCall(redeem, usergas, from, types.METARetryableTxAddress, common.Big0)
			} else {
	...
	case *types.metachainRetryTx:
		...
		prepaid := METAmath.BigMulByUint(evm.Context.BaseFee, tx.Gas)
...
func GetPosterGas(state *METAosState.METAosState, baseFee *big.Int, runMode core.MessageRunMode, posterCost *big.Int) uint64 {
	if runMode == core.MessageGasEstimationMode {
		...
		adjustedPrice := METAmath.BigMulByFrac(baseFee, 7, 8) // assume congestion
		if METAmath.BigLessThan(adjustedPrice, minGasPrice) {
			adjustedPrice = minGasPrice
		}
		baseFee = adjustedPrice

		// Pad the L1 cost in case the L1 gas price rises
		posterCost = METAmath.BigMulByBips(posterCost, GasEstimationL1PricePadding)
	}

	return METAmath.BigToUintSaturating(METAmath.BigDiv(posterCost, baseFee))
}
...
func (p *TxProcessor) GasChargingHook(gasRemaining *uint64) (common.Address, error) {
	...
	if basefee.Sign() > 0 && !p.msg.SkipL1Charging {
		...
		p.PosterFee = METAmath.BigMulByUint(basefee, p.posterGas) // round down
...
func (p *TxProcessor) EndTxHook(gasLeft uint64, success bool) {
	...
	if underlyingTx != nil && underlyingTx.Type() == types.metachainRetryTxType {
		inner, _ := underlyingTx.GetInner().(*types.metachainRetryTx)
		effectiveBaseFee := inner.GasFeeCap
		if p.msg.TxRunMode == core.MessageCommitMode && !METAmath.BigEquals(effectiveBaseFee, p.evm.Context.BaseFee) {
			log.Error(
				"metachainRetryTx GasFeeCap doesn't match basefee in commit mode",
		...
		gasRefund := METAmath.BigMulByUint(effectiveBaseFee, gasLeft)
		...
		refund := func(refundFrom common.Address, amount *big.Int) {
			...
			err = util.TransferBalance(&refundFrom, &inner.From, METAmath.BigSub(amount, toRefundAddr), p.evm, scenario, "refund")
			if err != nil {
				log.Error(errLog, "err", err, "feeAddress", refundFrom)
			}
		}
		...
		takeFunds(maxRefund, METAmath.BigMulByUint(effectiveBaseFee, gasUsed))
		...
		if p.state.METAOSVersion() >= 11 {
			...
			if infraFeeAccount != (common.Address{}) {
				...
				infraFee := METAmath.BigMin(minBaseFee, effectiveBaseFee)
				infraRefund := METAmath.BigMulByUint(infraFee, gasLeft)
		...
		if success {
			...
			tracingInfo := util.NewTracingInfo(p.evm, METAosAddress, p.msg.From, scenario)
			state := METAosState.OpenSystemMETAosStateOrPanic(p.evm.StateDB, tracingInfo, false)
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-METAmath.SaturatingCast(gasUsed)))
		return
	}

	basefee := p.evm.Context.BaseFee
	totalCost := METAmath.BigMul(basefee, METAmath.UintToBig(gasUsed)) // total cost = price of gas * gas burnt
	computeCost := METAmath.BigSub(totalCost, p.PosterFee)            // total cost = network's compute + poster's L1 costs
	...
	if p.state.METAOSVersion() > 4 {
		infraFeeAccount, err := p.state.InfraFeeAccount()
		p.state.Restrict(err)
		if infraFeeAccount != (common.Address{}) {
			minBaseFee, err := p.state.L2PricingState().MinBaseFeeWei()
			p.state.Restrict(err)
			infraFee := METAmath.BigMin(minBaseFee, basefee)
			computeGas := METAmath.SaturatingUSub(gasUsed, p.posterGas)
			infraComputeCost := METAmath.BigMulByUint(infraFee, computeGas)
			util.MintBalance(&infraFeeAccount, infraComputeCost, p.evm, scenario, purpose)
			computeCost = METAmath.BigSub(computeCost, infraComputeCost)
		}
	}
	if METAmath.BigGreaterThan(computeCost, common.Big0) {
	...
	if p.msg.GasPrice.Sign() > 0 { // in tests, gas price could be 0
		// METAOS's gas pool is meant to enforce the computational speed-limit.
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-METAmath.SaturatingCast(computeGas)))
	}
}
...
func (p *TxProcessor) ScheduledTxes() types.Transactions {
	...
	for _, log := range logs {
		if log.Address != METARetryableTxAddress || log.Topics[0] != RedeemScheduledEventID {
			continue
		}
		event := &precompilesgen.METARetryableTxRedeemScheduled{}
...
func (p *TxProcessor) L1BlockNumber(blockCtx vm.BlockContext) (uint64, error) {
	...
	tracingInfo := util.NewTracingInfo(p.evm, p.msg.From, METAosAddress, util.TracingDuringEVM)
	state, err := METAosState.OpenSystemMETAosState(p.evm.StateDB, tracingInfo, false)
	if err != nil {
		return 0, err
	}
