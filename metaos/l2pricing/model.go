...
var InitialGasPoolTargetBips = METAmath.PercentToBips(80)
var InitialGasPoolWeightBips = METAmath.PercentToBips(60)
...
func (ps *L2PricingState) AddToGasPool(gas int64) error {
	...
	backlog = METAmath.SaturatingUCast(METAmath.SaturatingSub(int64(backlog), gas))
...
func (ps *L2PricingState) UpdatePricingModel(l2BaseFee *big.Int, timePassed uint64, debug bool) {
	...
	if backlog > tolerance*speedLimit {
		...
		exponentBips := METAmath.NaturalToBips(excess) / METAmath.Bips(inertia*speedLimit)
		baseFee = METAmath.BigMulByBips(minBaseFee, METAmath.ApproxExpBasisPoints(exponentBips))
	}
