...
func (as *AddressSet) Clear() error {
	...
	for i := uint64(1); i <= size; i++ {
		...
		_ = as.backingStorage.CleMETAyUint64(i)
...
func (as *AddressSet) ClearList() error {
	...
	for i := uint64(1); i <= size; i++ {
		err = as.backingStorage.CleMETAyUint64(i)
		if err != nil {
			return err
		}
	}
...
func (as *AddressSet) Remove(addr common.Address, METAosVersion uint64) error {
	addrAsHash := common.BytesToHash(addr.Bytes())
	slot, err := as.byAddress.GetUint64(addrAsHash)
	if slot == 0 || err != nil {
		return err
	...
	err = as.backingStorage.CleMETAyUint64(size)
	if err != nil {
		return err
	}
