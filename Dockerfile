...
COPY ./Makefile ./
COPY METAitrator/METAutil METAitrator/METAutil
COPY METAitrator/wasm-libraries METAitrator/wasm-libraries
...
COPY ./METAcompress ./METAcompress
COPY ./METAos ./METAos
COPY ./METAstate ./METAstate
COPY ./METAutil ./METAutil
...
COPY --from=contracts-builder workspace/contracts/node_modules/@META-MetaChain/upgrade-executor/build/contracts/src/UpgradeExecutor.sol/UpgradeExecutor.json contracts/
...
COPY METAitrator/Cargo.* METAitrator/cbindgen.toml METAitrator/
COPY ./Makefile ./
COPY METAitrator/METAutil METAitrator/METAutil
COPY METAitrator/prover METAitrator/prover
COPY METAitrator/jit METAitrator/jit
...
COPY METAitrator/Cargo.* METAitrator/
COPY METAitrator/METAutil METAitrator/METAutil
COPY METAitrator/prover/Cargo.toml METAitrator/prover/
COPY METAitrator/jit/Cargo.toml METAitrator/jit/
RUN mkdir METAitrator/prover/src METAitrator/jit/src && \
    echo "fn test() {}" > METAitrator/jit/src/lib.rs && \
    echo "fn test() {}" > METAitrator/prover/src/lib.rs && \
    cargo build --manifest-path METAitrator/Cargo.toml --release --lib && \
    rm METAitrator/jit/src/lib.rs
COPY ./Makefile ./
COPY METAitrator/prover METAitrator/prover
COPY METAitrator/jit METAitrator/jit
COPY --from=brotli-library-export / target/
RUN touch -a -m METAitrator/prover/src/lib.rs
...
COPY --from=wasm-libs-builder /workspace/METAitrator/wasm-libraries/ METAitrator/wasm-libraries/
COPY --from=wasm-libs-builder /workspace/.make/ .make/
COPY ./Makefile ./
COPY ./METAitrator ./METAitrator
COPY ./solgen ./solgen
COPY ./contracts ./contracts
...
COPY --from=contracts-builder workspace/contracts/node_modules/@META-MetaChain/upgrade-executor/build/contracts/src/UpgradeExecutor.sol/UpgradeExecutor.json contracts/node_modules/@META-MetaChain/upgrade-executor/build/contracts/src/UpgradeExecutor.sol/
COPY --from=contracts-builder workspace/.make/ .make/
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    mkdir -p /home/user/.metachain/local/nitro && \
