...
var METAitratorValidationSteps = metrics.NewRegisteredHistogram("METAitrator/validation/steps", nil, metrics.NewBoundedHistogramSample())

type METAitratorSpawnerConfig struct {
...
type METAitratorSpawnerConfigFecher func() *METAitratorSpawnerConfig

var DefaultMETAitratorSpawnerConfig = METAitratorSpawnerConfig{
	Workers:             0,
...
func METAitratorSpawnerConfigAddOptions(prefix string, f *flag.FlagSet) {
	f.Int(prefix+".workers", DefaultMETAitratorSpawnerConfig.Workers, "number of concurrent validation threads")
	f.Duration(prefix+".execution-run-timeout", DefaultMETAitratorSpawnerConfig.ExecutionRunTimeout, "timeout before discarding execution run")
	f.String(prefix+".output-path", DefaultMETAitratorSpawnerConfig.OutputPath, "path to write machines to")
	MachineCacheConfigConfigAddOptions(prefix+".execution", f)
}

func DefaultMETAitratorSpawnerConfigFetcher() *METAitratorSpawnerConfig {
	return &DefaultMETAitratorSpawnerConfig
}

type METAitratorSpawner struct {
	stopwaiter.StopWaiter
	count         int32
	locator       *server_common.MachineLocator
	machineLoader *METAMachineLoader
	config        METAitratorSpawnerConfigFecher
}

func NewMETAitratorSpawner(locator *server_common.MachineLocator, config METAitratorSpawnerConfigFecher) (*METAitratorSpawner, error) {
	// TODO: preload machines
	spawner := &METAitratorSpawner{
		locator:       locator,
		machineLoader: NewMETAMachineLoader(&DefaultMETAitratorMachineConfig, locator),
		config:        config,
	}
	return spawner, nil
}

func (s *METAitratorSpawner) Start(ctx_in context.Context) error {
	s.StopWaiter.Start(ctx_in, s)
	return nil
}

func (s *METAitratorSpawner) LatestWasmModuleRoot() containers.PromiseInterface[common.Hash] {
	return containers.NewReadyPromise(s.locator.LatestWasmModuleRoot(), nil)
}

func (s *METAitratorSpawner) Name() string {
	return "METAitrator"
}

func (v *METAitratorSpawner) loadEntryToMachine(ctx context.Context, entry *validator.ValidationInput, mach *METAitratorMachine) error {
	resolver := func(ty METAutil.PreimageType, hash common.Hash) ([]byte, error) {
...
func (v *METAitratorSpawner) execute(
	...
	METAitratorValidationSteps.Update(int64(mach.GetStepCount()))
	if mach.IsErrored() {
...
func (v *METAitratorSpawner) Launch(entry *validator.ValidationInput, moduleRoot common.Hash) validator.ValidationRun {
...
func (v *METAitratorSpawner) Room() int {
...
//nolint:gosec
func (v *METAitratorSpawner) writeToFile(ctx context.Context, input *validator.ValidationInput, expOut validator.GoGlobalState, moduleRoot common.Hash) error {
...
func (v *METAitratorSpawner) WriteToFile(input *validator.ValidationInput, expOut validator.GoGlobalState, moduleRoot common.Hash) containers.PromiseInterface[struct{}] {
...
func (v *METAitratorSpawner) CreateExecutionRun(wasmModuleRoot common.Hash, input *validator.ValidationInput) containers.PromiseInterface[validator.ExecutionRun] {
...
func (v *METAitratorSpawner) Stop() {
	v.StopOnly()
}
