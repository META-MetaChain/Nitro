...
type Router struct {
	...
	confirmedSequenceNumberChan chan METAutil.MessageIndex

	forwardTxStreamer       broadcastclient.TransactionStreamerInterface
	forwardConfirmationChan chan METAutil.MessageIndex
}
...
func NewBroadcastClients(
	...
	l2ChainId uint64,
	currentMessageCount METAutil.MessageIndex,
	txStreamer broadcastclient.TransactionStreamerInterface,
	confirmedSequenceNumberListener chan METAutil.MessageIndex,
	...
	newStandardRouter := func() *Router {
		return &Router{
			...
			confirmedSequenceNumberChan: make(chan METAutil.MessageIndex, ROUTER_QUEUE_SIZE),
...
func (bcs *BroadcastClients) Start(ctx context.Context) {
	...
	var lastConfirmed METAutil.MessageIndex
	recentFeedItemsNew := make(map[METAutil.MessageIndex]time.Time, RECENT_FEED_INITIAL_MAP_SIZE)
	recentFeedItemsOld := make(map[METAutil.MessageIndex]time.Time, RECENT_FEED_INITIAL_MAP_SIZE)
	bcs.primaryRouter.LaunchThread(func(ctx context.Context) {
		...
		confSeqHandler := func(cs METAutil.MessageIndex, router *Router) {
			if cs == lastConfirmed {
				return
			}
			lastConfirmed = cs
