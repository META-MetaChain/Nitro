...
var (
	clientsCurrentGauge              = metrics.NewRegisteredGauge("META/feed/clients/current", nil)
	clientsConnectCount              = metrics.NewRegisteredCounter("META/feed/clients/connect", nil)
	clientsDisconnectCount           = metrics.NewRegisteredCounter("META/feed/clients/disconnect", nil)
	clientsTotalSuccessCounter       = metrics.NewRegisteredCounter("META/feed/clients/success", nil)
	clientsTotalFailedUpgradeCounter = metrics.NewRegisteredCounter("META/feed/clients/failed/upgrade", nil)
	clientsTotalFailedWorkerCounter  = metrics.NewRegisteredCounter("META/feed/clients/failed/worker", nil)
	clientsDurationHistogram         = metrics.NewRegisteredHistogram("META/feed/clients/duration", nil, metrics.NewBoundedHistogramSample())
)
...
func (cm *ClientManager) doBroadcast(bm *m.BroadcastMessage) ([]*ClientConnection, error) {
	...
	for client := range cm.clientPtrMap {
		...
		var seqNum *METAutil.MessageIndex
		n := len(bm.Messages)
		if n == 0 {
			seqNum = nil
		} else if n == 1 {
