...
var (
	...
	HTTPHeaderFeedServerVersion       = textproto.CanonicalMIMEHeaderKey("metachain-Feed-Server-Version")
	HTTPHeaderFeedClientVersion       = textproto.CanonicalMIMEHeaderKey("metachain-Feed-Client-Version")
	HTTPHeaderRequestedSequenceNumber = textproto.CanonicalMIMEHeaderKey("metachain-Requested-Sequence-Number")
	HTTPHeaderChainId                 = textproto.CanonicalMIMEHeaderKey("metachain-Chain-Id")
	upgradeToWSTimer                  = metrics.NewRegisteredTimer("META/feed/clients/upgrade/duration", nil)
	startWithHeaderTimer              = metrics.NewRegisteredTimer("META/feed/clients/start/duration", nil)
)
...
func (s *WSBroadcastServer) StartWithHeader(ctx context.Context, header ws.HandshakeHeader) error {
	...
	handle := func(conn net.Conn) {
		...
		var requestedSeqNum METAutil.MessageIndex
		upgrader := ws.Upgrader{
			...
			OnHeader: func(key []byte, value []byte) error {
				...
				if headerName == HTTPHeaderFeedClientVersion {
					...
					requestedSeqNum = METAutil.MessageIndex(num)
	...
	log.Info("metachain websocket broadcast server is listening", "address", ln.Addr().String())
