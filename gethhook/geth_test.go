...
var testChainConfig = &params.ChainConfig{
	...
	metachainChainParams: params.metachainDevTestParams(),
}
...
func TestEthDepositMessage(t *testing.T) {
	...
	header := METAostypes.L1IncomingMessageHeader{
		Kind:        METAostypes.L1MessageType_EthDeposit,
		Poster:      addr,
		BlockNumber: 864513,
	...
	msg2 := METAostypes.L1IncomingMessage{
		Header: &header,
		L2msg:  msgBuf2.Bytes(),
	}
...
func RunMessagesThroughAPI(t *testing.T, msgs [][]byte, statedb *state.StateDB) {
	...
	for _, data := range msgs {
		msg, err := METAostypes.ParseIncomingL1Message(bytes.NewReader(data), nil)
		if err != nil {
			t.Error(err)
		}
		txes, err := METAos.ParseL2Transactions(msg, chainId, nil)
		if err != nil {
			t.Error(err)
		}
		...
		METAos.FinalizeBlock(nil, nil, statedb, testChainConfig)
	}
}
