...
type METAosPrecompileWrapper struct {
	inner precompiles.METAosPrecompile
}

func (p METAosPrecompileWrapper) RequiredGas(input []byte) uint64 {
	panic("Non-advanced precompile method called")
}

func (p METAosPrecompileWrapper) Run(input []byte) ([]byte, error) {
	panic("Non-advanced precompile method called")
}

func (p METAosPrecompileWrapper) RunAdvanced(
	input []byte,
	gasSupplied uint64,
...
func init() {
	core.ReadyEVMForL2 = func(evm *vm.EVM, msg *core.Message) {
		if evm.ChainConfig().Ismetachain() {
			evm.ProcessingHook = METAos.NewTxProcessor(evm, msg)
		}
	}

	for k, v := range vm.PrecompiledContractsBerlin {
		vm.PrecompiledAddressesmetachain = append(vm.PrecompiledAddressesmetachain, k)
		vm.PrecompiledContractsmetachain[k] = v
	}
	...
	for addr, precompile := range precompiles.Precompiles() {
		...
		var wrapped vm.AdvancedPrecompile = METAosPrecompileWrapper{precompile}
		vm.PrecompiledContractsmetachain[addr] = wrapped
		vm.PrecompiledAddressesmetachain = append(vm.PrecompiledAddressesmetachain, addr)
	}
