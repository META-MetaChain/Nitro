...
import (
	...
	"github.com/META-MetaChain/nitro/zeroheavy"
)
...
type InboxBackend interface {
	...
	ReadDelayedInbox(seqNum uint64) (*METAostypes.L1IncomingMessage, error)
}
...
func parseSequencerMessage(ctx context.Context, batchNum uint64, data []byte, dasReader DataAvailabilityReader, keysetValidationMode KeysetValidationMode) (*sequencerMessage, error) {
	...
	if len(payload) > 0 && IsBrotliMessageHeaderByte(payload[0]) {
		decompressed, err := METAcompress.Decompress(payload[1:], MaxDecompressedLen)
		if err == nil {
...
func RecoverPayloadFromDasBatch(
	...
	if preimages != nil {
		if preimages[METAutil.Keccak256PreimageType] == nil {
			preimages[METAutil.Keccak256PreimageType] = make(map[common.Hash][]byte)
		}
...
func NewInboxMultiplexer(backend InboxBackend, delayedMessagesRead uint64, dasReader DataAvailabilityReader, keysetValidationMode KeysetValidationMode) METAostypes.InboxMultiplexer {
...
func (r *inboxMultiplexer) Pop(ctx context.Context) (*METAostypes.MessageWithMetadata, error) {
...
func (r *inboxMultiplexer) getNextMsg() (*METAostypes.MessageWithMetadata, error) {
	...
	kind := segment[0]
	segment = segment[1:]
	var msg *METAostypes.MessageWithMetadata
	if kind == BatchSegmentKindL2Message || kind == BatchSegmentKindL2MessageBrotli {

		if kind == BatchSegmentKindL2MessageBrotli {
			decompressed, err := METAcompress.Decompress(segment, METAostypes.MaxL2MessageSize)
			if err != nil {
